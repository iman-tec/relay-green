...
import {
  ...
  PanelLeftOpen, PanelLeftClose, PanelRightOpen, PanelRightClose, AlertTriangle,
  FileText, Paperclip,
} from "lucide-react";
...
export function EngineerSessionClient({ sessionId }: { sessionId: string }) {
  ...
  return (
    ...
    >
      ...
      <IntakeTray sessionId={sessionId} />
...
// ── Intake tray ────────────────────────────────────────────────────────────
// Right-side collapsible panel showing the bot↔customer intake transcript
// summary the customer produced while ringing. The summary is generated by
// the summarize-intake edge function on engineer accept; if it hasn't landed
// yet (or the edge call failed) the tray falls back to the raw transcript.
//
// State source: public.client_intakes joined on guest_call_id = sessionId.
// Already covered by the existing "Staff read intakes" RLS policy, so the
// engineer's auth context can SELECT directly.
...
type IntakeRow = {
  ...
  intake_summary_updated_at: string | null;
  intake_messages:
    | Array<{
        role: "assistant" | "user";
        body: string;
...
const TRAY_STORAGE_KEY = "relay-intake-tray-open";

function IntakeTray({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem(TRAY_STORAGE_KEY, open ? "1" : "0");
    }
  }, [open]);
  ...
  useEffect(() => {
    ...
    const fetchIntake = async () => {
      const { data } = await sb
        .from("client_intakes")
        .select(
          "id, familiarity, ai_tools_used, developing, technologies, intake_summary, intake_summary_updated_at, intake_messages",
        )
        .eq("guest_call_id", sessionId)
        .order("created_at", { ascending: false })
        .limit(1)
        .maybeSingle();
      if (!cancelled) setIntake((data as IntakeRow | null) ?? null);
    };

    void fetchIntake();
    // Realtime: the summarize-intake edge fn updates intake_summary after
    // accept; we want to swap from "fetching summary…" → real summary
    // without a manual reload.
    const channel = sb
      .channel(`intake-tray:${sessionId}`)
      .on(
        "postgres_changes",
  ...
  if (!intake) {
    return (
      <aside
        aria-label="Intake brief"
        className={`shrink-0 border-l border-[var(--border)] bg-[var(--surface)] transition-[width] duration-200 ease-out ${open ? "w-72" : "w-10"}`}
      >
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          aria-label={open ? "Collapse intake brief" : "Expand intake brief"}
          className="flex h-10 w-10 items-center justify-center text-[var(--text-muted)] hover:text-[var(--text)]"
        >
          {open ? <PanelRightClose size={16} /> : <PanelRightOpen size={16} />}
        </button>
        {open && (
          <div className="px-4 pb-4 pt-1 text-xs text-[var(--text-muted)]">
            No intake brief for this session.
          </div>
        )}
      </aside>
    );
  ...
  const messages = Array.isArray(intake.intake_messages) ? intake.intake_messages : [];
  const userTurns = messages.filter((m) => m.role === "user");
  const attachments = messages
    .map((m) => m.attachment)
    .filter((a): a is { name: string; mime: string } => Boolean(a));
  const hasSummary = Boolean(intake.intake_summary);
  // First line of summary doubles as the "headline" badge at the top.
  const summaryLines = (intake.intake_summary ?? "").split("\n");
  const headline = summaryLines[0]?.trim() ?? "";
  const summaryBody = summaryLines.slice(1).join("\n").trim();

  return (
    <aside
      aria-label="Intake brief"
      className={`shrink-0 border-l border-[var(--border)] bg-[var(--surface)] transition-[width] duration-200 ease-out ${open ? "w-80" : "w-10"}`}
    >
      <div className="flex items-center justify-between border-b border-[var(--border)] px-3 py-2">
        ...
        {open && (
          <span className="inline-flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-[var(--text-muted)]">
            <FileText size={12} /> Intake brief
          </span>
        )}
      </div>

      {open && (
        <div className="flex h-[calc(100vh-40px)] flex-col gap-3 overflow-y-auto px-3 py-3">
          {hasSummary && headline && (
          ...
          <div className="flex flex-wrap gap-1.5">
            {intake.developing && <IntakeChip label="Building" value={intake.developing} />}
            {intake.familiarity && <IntakeChip label="Familiarity" value={intake.familiarity} />}
            {intake.ai_tools_used && <IntakeChip label="AI" value={intake.ai_tools_used} />}
            {intake.technologies?.length ? (
              <IntakeChip label="Stack" value={intake.technologies.join(", ")} />
            ) : null}
          </div>

          {hasSummary && summaryBody && (
          ...
          {!hasSummary && userTurns.length > 0 && (
            <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] px-3 py-2 text-xs text-[var(--text-muted)]">
              Summary is being generated… falling back to the raw transcript below.
            </div>
          )}
            ...
            <div>
              ...
              <ul className="space-y-1">
                {attachments.map((a, i) => (
                  <li
                    key={`${a.name}_${i}`}
                    className="inline-flex w-full items-center gap-2 truncate rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs text-[var(--text)]"
                  >
                    <Paperclip size={12} className="text-[var(--text-muted)]" />
                    <span className="truncate">{a.name}</span>
                    <span className="ml-auto text-[10px] text-[var(--text-faint)]">{a.mime}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {messages.length > 0 && (
            <div>
              <button
                ...
                className="text-[11px] font-medium uppercase tracking-wider text-[var(--text-muted)] hover:text-[var(--text)]"
              >
                {showTranscript ? "Hide" : "Show"} full transcript ({messages.length})
              </button>
              {showTranscript && (
...
function IntakeChip({ label, value }: { label: string; value: string }) {
  return (
    <span
      className="inline-flex items-center gap-1 rounded-full border border-[var(--border)] bg-[var(--surface-raised)] px-2 py-0.5 text-[11px] text-[var(--text)]"
      title={`${label}: ${value}`}
    >
