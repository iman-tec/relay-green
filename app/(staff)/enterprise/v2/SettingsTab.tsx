...
export function SettingsTab() {
  ...
  const staff = useApiData<{ members: Member[] }>(
    "/api/enterprise/users?scope=staff"
  );
  const prefsFetch = useApiData<PrefsPayload>(
    "/api/enterprise/notification-prefs"
  );
  ...
  const humanOrgError = (code: string) => {
    switch (code) {
      ...
      case "invalid_domain":
        return 'That doesn\'t look like a valid domain (e.g. "acme.com").';
      case "domain_taken":
        return "Another organisation already uses that domain.";
      case "invalid_retention":
        return "Pick one of the available retention windows.";
  ...
  const requestExport = useCallback(async () => {
    ...
    try {
      ...
      setNote(
        `Couldn't export (${e instanceof Error ? e.message : "network error"}).`
      );
    } finally {
      setExporting(false);
    }
  ...
  const invite = async () => {
    if (!iEmail.trim() || !iName.trim()) {
      setIErr("Name and email required.");
      return;
    }
    setIBusy(true);
    setIErr(null);
    try {
      ...
      setIErr(e instanceof Error ? e.message : "Invite failed");
    } finally {
      setIBusy(false);
    }
  };
    ...
    <TabBody>
      ...
      <div className="flex flex-col gap-5">
        <SettingsSection icon={<Building2 size={16} />} title="Organization">
          <IdentityBlock
            name={org?.name ?? ""}
            sub={org?.primaryDomain ?? org?.status}
          />
          <EditableField
            ...
            onSave={async (v) => {
              ...
              if (res.ok) {
                setNote("Organisation name saved.");
                me.reload();
              } else {
                setNote(humanOrgError(res.error));
              }
            }}
          />
          <EditableField
            ...
            onSave={async (v) => {
              ...
              if (res.ok) {
                setNote(
                  v.trim() === ""
                    ? "Primary domain cleared."
                    : "Primary domain saved."
                );
                me.reload();
              } else {
                setNote(humanOrgError(res.error));
          ...
          {me.data?.channelPartner ? (
            <>
              <EditableField
                ...
                value={me.data.channelPartner.name}
                readOnly
                hint="The partner who onboarded your organization."
              />
              <EditableField
                label="Partner discount"
                value={`${me.data.channelPartner.discountPct}%`}
                readOnly
                hint="Discount applied to your usage through this partner."
              />
            </>
          ) : null}
          {org?.discountPct ? (
        ...
        <SettingsSection
          icon={<Users size={16} />}
          title="Internal team"
          desc="Admins who manage this organization."
        >
          ...
          {staff.loading ? (
            ...
            <EmptyState
              ...
              body="Invite teammates to co-manage the org."
            />
          ) : (
            <ul>
              {admins.map((m) => (
        ...
        <SettingsSection icon={<Bell size={16} />} title="Notifications">
          <SettingsToggle
            ...
            desc="When a member starts a session."
            on={notif.sessions}
            onChange={(v) => setNotif({ ...notif, sessions: v })}
          />
          <SettingsToggle
            label="Low-minutes warning"
            desc="When an org or department pool runs low."
            on={notif.lowMinutes}
            onChange={(v) => setNotif({ ...notif, lowMinutes: v })}
          />
          <SettingsToggle
            label="Weekly usage digest"
            desc="A Monday summary email."
            on={notif.weekly}
        ...
        <SettingsSection
          ...
          title="Privacy & data"
          desc="You are the data controller for this organization."
          accent
        >
          <div
            className="flex flex-col gap-2 rounded-xl border bg-[var(--surface)] p-4 sm:flex-row sm:items-center sm:justify-between"
            style={{ borderColor: "var(--border)" }}
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: "var(--text-muted)" }}>
                  How long session records are kept before scheduled purge.
                </div>
              </div>
            </div>
          ...
          <div
            className="mt-3 flex flex-col gap-2 rounded-xl border bg-[var(--surface)] p-4 sm:flex-row sm:items-center sm:justify-between"
            style={{ borderColor: "var(--border)" }}
          >
            <div className="flex items-start gap-3">
              <Download
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: "var(--text-muted)" }}>
                  Portable .zip with CSVs for org, departments, members,
                  sessions, usage and billing.
                </div>
              </div>
            </div>
            <Button
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: "var(--text-muted)" }}>
                  Right-to-erasure strips a member&apos;s name, avatar and email
                  visibility while preserving aggregate session records for
                  billing. Trigger it per member from the{" "}
                  <strong style={{ color: "var(--text)" }}>Members</strong> tab.
                </div>
              </div>
            </div>
          </div>
        ...
        <SettingsSection icon={<KeyRound size={16} />} title="Single sign-on">
          <p
            ...
            style={{ color: "var(--text-muted)" }}
          >
            SAML / OIDC single sign-on is configured by Relay. Contact support
            to enable SSO for your domain.
          </p>
        </SettingsSection>
      ...
      <Modal
        ...
        description="They can co-manage this organization."
        footer={
