...
/*
 * Enterprise-scoped Add Department drawer. POSTs to
 * /api/enterprise/departments — invites a dept admin, transfers minutes
 * from the enterprise pool to the new dept pool.
 */
...
export function AddDepartmentDrawer({
  open,
  enterpriseRemainingMinutes,
  onClose,
  onCreated,
}: {
  open:                       boolean;
  /** Used to surface "X min available" hint. */
  enterpriseRemainingMinutes?: number;
  onClose:                    () => void;
  ...
  const submit = async () => {
    ...
    try {
      ...
      const body = (await res.json().catch(() => ({}))) as {
        department?: { id: string }; error?: string;
      };
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <Field label="Initial minutes from the enterprise pool">
          ...
          {typeof enterpriseRemainingMinutes === "number" && (
            <span className="text-[11px]" style={{ color: "var(--text-muted)" }}>
              Available in the enterprise pool: {enterpriseRemainingMinutes.toLocaleString()} min
            </span>
          )}
        </Field>
