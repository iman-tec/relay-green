...
import { useApiData, eur, StatCard, LoadingState, ErrorState } from "./_shared";
...
export function BillingTab() {
  const { data, loading, error, reload } = useApiData<Billing>(
  ...
  const exportStatement = () => {
    ...
    for (const t of tx) {
      lines.push(
        `${new Date(t.occurredAt).toISOString().slice(0, 10)},"${t.label}",${t.durationMin},${(t.amountCents / 100).toFixed(2)}`
      );
    }
    ...
    a.href = url;
    a.download = "relay-statement.csv";
    a.click();
  ...
  return (
    <section>
      ...
      <p className="mb-6 text-sm" style={{ color: "var(--text-muted)" }}>
        Pay-per-minute · €{(data.revenue.perMinuteCents / 100).toFixed(2)}/min,
        billed by the second. No subscription.
      </p>

      <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-3">
        <StatCard
          icon={<Receipt size={16} />}
          value={eur(data.revenue.thisMonthCents)}
          label="Spend this month"
        />
        <StatCard
          icon={<Receipt size={16} />}
          value={eur(data.revenue.last30DaysCents)}
          label="Last 30 days"
        />
        <StatCard
          icon={<Receipt size={16} />}
          value={eur(data.revenue.lifetimeCents)}
          label="Lifetime"
        />
      </div>
      ...
      >
        <div className="flex items-start justify-between gap-3">
          ...
          <StatusBadge
            compact
            tone={data.plan.status === "active" ? "ok" : "neutral"}
          >
            {data.plan.status}
          </StatusBadge>
        </div>
        ...
        {tx.length === 0 ? (
          <div className="p-6">
            <EmptyState
              ...
              body="Charges appear here as sessions complete."
            />
          </div>
        ) : (
          <ul>
