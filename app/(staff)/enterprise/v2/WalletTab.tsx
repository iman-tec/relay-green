...
type Bundle = { code: string; label: string; minutes: number; amountCents: number };
type WalletData = {
  currency: string;
  ...
  return (
    <TabBody>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        ...
        <StatCard icon={<Layers size={16} />} value={num(data.distributedMinutes)} label="Distributed to depts" />
        <StatCard icon={<TrendingDown size={16} />} value={num(free)} label="Undistributed" />
      </div>

      {low && (
        <div className="mt-4 rounded-xl border px-4 py-3 text-sm"
          style={{ borderColor: "var(--warn)", background: "color-mix(in srgb, var(--warn) 10%, transparent)", color: "var(--text)" }}>
          Low balance — {num(data.remainingMinutes)} minutes left. Top up to keep sessions running.
        </div>
      )}

      <section className="mt-6">
        <h2 className="mb-3 text-sm font-semibold" style={{ color: "var(--text)" }}>Buy minutes</h2>
              ...
              return (
                <div key={b.code} className="flex flex-col rounded-2xl border p-5"
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <span className="font-serif text-lg tabular-nums" style={{ color: "var(--text)" }}>{eur(b.amountCents)}</span>
                    <Button onClick={() => setBuying(b)}>Buy</Button>
                  </div>
                </div>
              );
            })}
...
function BuyBundleModal({ bundle, onClose, onCredited }: {
  bundle: Bundle; onClose: () => void; onCredited: () => void;
}) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (alive) setFetchError(e instanceof Error ? e.message : "Couldn't start checkout.");
      }
    })();
    return () => { alive = false; };
  }, [bundle.code]);
        ...
        <div className="flex items-start gap-3 border-b px-5 py-4" style={{ borderColor: "var(--border)" }}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[12px]" style={{ color: "var(--text-muted)" }}>
              {eur(bundle.amountCents)} charged once. Minutes are added to your company pool immediately.
            </p>
          </div>
        ...
        <div className="px-5 py-4">
          {fetchError && (
            <div className="rounded-md border px-3 py-2 text-[12px]"
              style={{ borderColor: "color-mix(in srgb, var(--risk) 30%, transparent)", backgroundColor: "color-mix(in srgb, var(--risk) 8%, transparent)", color: "var(--risk)" }}>
              {fetchError}
            </div>
          )}
          ...
          {!fetchError && clientSecret && stripePromise && paymentIntentId && (
            <Elements key={theme} stripe={stripePromise} options={{ clientSecret, appearance }}>
              <BuyForm bundle={bundle} paymentIntentId={paymentIntentId} onCancel={onClose} onCredited={onCredited} />
            </Elements>
          )}
          {!fetchError && clientSecret && !stripePromise && (
            <div className="rounded-md border px-3 py-2 text-[12px]"
              style={{ borderColor: "color-mix(in srgb, var(--warn) 30%, transparent)", backgroundColor: "color-mix(in srgb, var(--warn) 8%, transparent)", color: "var(--warn)" }}>
...
function BuyForm({ bundle, paymentIntentId, onCancel, onCredited }: {
  bundle: Bundle; paymentIntentId: string; onCancel: () => void; onCredited: () => void;
}) {
  ...
  const pay = async () => {
    ...
    if (error) { setErrMsg(error.message ?? "Payment failed."); setSubmitting(false); return; }
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error ?? "Payment took, but crediting failed — contact support.");
      onCredited();
    } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="mt-1 flex items-center justify-end gap-2">
        ...
        <Button onClick={() => void pay()} loading={submitting} iconLeft={submitting ? undefined : <Check size={14} />}>
