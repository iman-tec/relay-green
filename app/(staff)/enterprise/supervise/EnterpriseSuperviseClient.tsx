...
const HEALTH_TOKENS: Record<
  Health,
  { bar: string; pill_bg: string; pill_fg: string; label: string }
> = {
  green: {
    bar: BRAND_GREEN,
    pill_bg: BRAND_GREEN_SOFT,
    pill_fg: BRAND_GREEN,
    label: "Healthy",
  },
  amber: {
    bar: URGENT_AMBER,
    pill_bg: URGENT_AMBER_SOFT,
    pill_fg: URGENT_AMBER,
    label: "Watch",
  },
  red: {
    bar: CRIT_RED,
    pill_bg: CRIT_RED_SOFT,
    pill_fg: CRIT_RED,
    label: "At risk",
  },
};
...
export function EnterpriseSuperviseClient() {
  ...
  useEffect(() => {
    ...
    const fetchOnce = async () => {
      const res = await fetch("/api/enterprise/sessions?limit=200", {
        cache: "no-store",
      });
    ...
    <div className="mx-auto max-w-screen-2xl space-y-6 px-6 py-8">
      <div>
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>
          Live, waiting, and past calls from your organization. The colored bar
      ...
      {loading ? (
        ...
        >
          ...
          <p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>
            {tab === "live" &&
              "No active calls in your org right now. New activity appears here in real time."}
            {tab === "waiting" && "No calls queued to be picked up."}
            {tab === "past" &&
              "Once your users have a session, it'll appear here once it ends."}
          </p>
        </div>
      ) : (
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {visible.map((s) => (
            <SessionCard key={s.id} session={s} isPast={PAST.has(s.status)} />
          ))}
        </div>
      )}
    </div>
