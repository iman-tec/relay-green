...
export function DeptUsageTab() {
  const { data, loading, error, reload } = useApiData<{ byPeriod: Period[] }>("/api/department/usage");
  ...
  const exportCsv = () => {
    ...
    a.href = url; a.download = "department-usage.csv"; a.click();
        ...
        {byPeriod.length === 0 ? (
          ...
          <>
            <div className="flex items-end gap-3 overflow-x-auto px-4 py-6">
              {byPeriod.map((p) => {
                const h = p.minutes != null ? Math.max(4, Math.round((p.minutes / maxMin) * 120)) : 8;
                return (
                  <div key={p.period} className="flex min-w-[48px] flex-col items-center gap-2">
                    ...
                    <span className="text-[10px] tabular-nums" style={{ color: "var(--text-muted)" }}>{p.period.slice(2)}</span>
                  </div>
                );
              })}
            </div>
            <ul className="border-t" style={{ borderColor: "var(--border)" }}>
              {byPeriod.map((p) => (
                <li key={p.period} className="flex items-center justify-between border-t px-4 py-2.5 first:border-t-0" style={{ borderColor: "var(--border)" }}>
                  <span className="text-sm" style={{ color: "var(--text)" }}>{p.period}</span>
                  {p.suppressed ? (
                    ...
                    <span className="text-sm tabular-nums" style={{ color: "var(--text-muted)" }}>
                      {num(p.minutes)}m · {p.sessions} sessions · {eur(p.spendCents)}
                    </span>
                  )}
                </li>
              ))}
