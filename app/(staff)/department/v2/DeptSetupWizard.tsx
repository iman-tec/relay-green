...
export function DeptSetupWizard({
  open, onClose, deptName, allocatedMinutes, onChanged,
}: {
  open: boolean;
  onClose: () => void;
  deptName: string;
  ...
  const close = () => {
    setStep(1); setEmail(""); setName(""); setInvited([]); setErr(null); onClose();
  };
  ...
  const invite = async () => {
    ...
    try {
      const res = await fetch("/api/department/employees", {
        ...
        body: JSON.stringify({ name: name.trim(), email: email.trim().toLowerCase(), allocatedMinutes: 0 }),
      });
  ...
  const footer = (
    <div className="flex items-center justify-between gap-2">
      <span className="text-xs" style={{ color: "var(--text-muted)" }}>Step {step} of 2</span>
      <div className="flex gap-2">
        {step > 1 && <Button variant="ghost" onClick={() => { setErr(null); setStep(1); }} disabled={busy}>Back</Button>}
        {step < 2
          ? <Button onClick={() => { setErr(null); setStep(2); }}>Continue</Button>
    ...
    <Modal open={open} onClose={close} title={titles[step - 1]} description={descs[step - 1]} footer={footer}>
      {step === 1 ? (
        <div className="flex flex-col items-center gap-4 py-2 text-center">
          ...
          <div className="w-full rounded-xl border p-4" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="mt-1 font-serif text-3xl tabular-nums" style={{ color: "var(--text)" }}>
              {num(allocatedMinutes)}<span className="ml-1 text-sm" style={{ color: "var(--text-muted)" }}>min</span>
            </div>
            <p className="mt-1 text-xs" style={{ color: "var(--text-faint)" }}>
              You distribute these minutes to employees. To raise the budget, ask your enterprise admin.
            </p>
          </div>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          ...
          <Button variant="secondary" onClick={() => void invite()} loading={busy} iconLeft={<Mail size={14} />}>Send invite</Button>
        </div>
      )}
    </Modal>
  );
