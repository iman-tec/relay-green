...
export function DeptSettingsTab() {
  const { data, loading, error, reload } = useApiData<Employees>(
    "/api/department/employees"
  );
  const prefsFetch = useApiData<{ prefs: NotifPrefs }>(
    "/api/department/notification-prefs"
  );
    ...
    <TabBody>
      ...
      <div className="flex flex-col gap-5">
        <SettingsSection icon={<Building2 size={16} />} title="Department">
          <IdentityBlock
            name={d?.name ?? ""}
            sub={`${data?.enterprise.name ?? ""} · ${num(memberCount)} members`}
          />
          ...
          <EditableField
            ...
            onSave={async (v) => {
              const next = v.trim();
              if (!next) {
                setNote("Department name can't be empty.");
                return;
              }
              ...
              if (res.ok) {
                setNote("Department name updated.");
                reload();
              } else {
          ...
          >
            ...
            <p
              ...
              style={{ color: "var(--text-muted)" }}
            >
              Your minute pool + department structure are managed by your
              enterprise admin.
            </p>
          </div>
        </SettingsSection>
        ...
        <SettingsSection icon={<Users size={16} />} title="Team">
          <p
            ...
            style={{ color: "var(--text-muted)" }}
          >
            Invite + manage your {num(memberCount)} team members from the{" "}
            <strong style={{ color: "var(--text)" }}>Team members</strong> tab.
          </p>
        </SettingsSection>
        ...
        <SettingsSection icon={<Bell size={16} />} title="Notifications">
          <SettingsToggle
            ...
            desc="When a team member starts a session."
            on={notif.sessions}
            onChange={(v) => setNotif({ ...notif, sessions: v })}
          />
          <SettingsToggle
            label="Low-minutes warning"
            desc="When the department pool runs low."
            on={notif.lowMinutes}
            onChange={(v) => setNotif({ ...notif, lowMinutes: v })}
          />
          <SettingsToggle
            label="New member joined"
            desc="When someone is added to the department."
            on={notif.newMember}
            onChange={(v) => setNotif({ ...notif, newMember: v })}
          />
          <div className="mt-3">
            <Button size="sm" disabled={savingNotif} onClick={savePrefs}>
