...
export function SalesTab() {
  ...
  const months = useMemo(() => {
    ...
    for (const e of data?.enterprises ?? []) {
      ...
      if (m.has(key)) {
        const b = m.get(key)!;
        b.acquired += 1;
        b.commissionCents += Math.round(
          e.usedMinutes * CENTS_PER_MINUTE * commissionPct
        );
      }
    }
  ...
  const totalCommission = (data?.enterprises ?? []).reduce(
    (s, e) => s + Math.round(e.usedMinutes * CENTS_PER_MINUTE * commissionPct),
    0
  );
  ...
  const share = async () => {
    ...
    if (nav.share) {
      try {
        await nav.share({
          title: "Join Relay",
          text: `Get a human engineer in 90 seconds — via ${data?.reseller.name}`,
          url: refLink,
        });
      } catch {
        /* cancelled */
    ...
    <TabBody>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        <StatCard
          ...
          value={num(data?.reseller.totalEnterprises)}
          label="Clients acquired"
          hint={`${num(data?.reseller.activeEnterprises)} active`}
        />
        <StatCard
        ...
        <StatCard
          ...
          value={`${num(data?.reseller.commission)}%`}
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        >
          ...
          <div
            className="flex flex-col items-center gap-3 rounded-xl border bg-[var(--surface)] p-4"
          ...
          <div className="mt-3 flex flex-col gap-2">
            <button
              type="button"
              onClick={() => copy("link")}
              className="inline-flex items-center justify-center gap-2 rounded-lg border bg-[var(--surface)] px-3 py-2 text-xs transition-colors hover:bg-[var(--surface-raised)]"
            ...
            <button
              type="button"
              onClick={() => void share()}
              className="inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-white"
          ...
          <p
            className="mt-3 text-center text-[11px] break-all"
          ...
          {(data?.enterprises ?? []).length === 0 ? (
            <div className="p-6">
              <EmptyState
                ...
                body="Share your code or QR to start acquiring clients."
              />
            </div>
          ) : (
            <>
              <div
                className="flex items-end justify-around gap-2 px-4 py-6"
                style={{ minHeight: 180 }}
              >
                {months.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className="flex w-full max-w-[40px] items-end"
                        style={{ height: 130 }}
                      >
                        <div
                          className="w-full rounded-t transition-all"
                          style={{
                            height: Math.max(m.acquired ? 6 : 2, h),
                            background: m.acquired
                              ? "var(--primary)"
                              : "var(--surface-raised)",
                          }}
                          title={`${m.acquired} acquired · ${eur(m.commissionCents)} commission`}
                        />
                      </div>
                      <span
              ...
              <ul className="border-t" style={{ borderColor: "var(--border)" }}>
                {months
                  .filter((m) => m.acquired > 0 || m.commissionCents > 0)
                  .reverse()
                  .map((m) => (
                    <li
                      key={m.period}
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{ color: "var(--text-muted)" }}
                      >
                        {num(m.acquired)} client{m.acquired === 1 ? "" : "s"} ·{" "}
                        {eur(m.commissionCents)} commission
                      </span>
                    </li>
                  ))}
              </ul>
