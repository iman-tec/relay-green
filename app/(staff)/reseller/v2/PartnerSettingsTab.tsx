...
export function PartnerSettingsTab() {
  ...
  const prefsFetch    = useApiData<PrefsPayload>("/api/reseller/notification-prefs");
  ...
  const addMember = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/reseller/team-members", {
        ...
        body:    JSON.stringify({ email, fullName: name || undefined, role: iRole }),
      });
  ...
  const removeMember = useCallback(async (id: string) => {
    const res = await fetch(`/api/reseller/team-members/${id}`, { method: "DELETE" });
    if (res.ok) {
      teamFetch.reload();
  ...
  }, [whiteLabel, brandColor, brandDisplayName, brandSupportEmail, brandingFetch]);
  ...
  const savePrefs = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const body = (await res.json().catch(() => ({}))) as { error?: string };
        setNote(`Couldn't save preferences (${body.error ?? "unknown error"}).`);
      } else {
    ...
    <TabBody>
      ...
      <div className="flex flex-col gap-5">
        ...
        <SettingsSection icon={<Users size={16} />} title="Internal team" desc="Your own team members with access to this partner account.">
          <div className="-mt-1 mb-3 flex justify-end">
            <Button size="sm" iconLeft={<UserPlus size={14} />} onClick={() => { setInviteError(null); setInviteOpen(true); }}>
              Add member
            </Button>
          </div>
          {teamFetch.loading && !teamFetch.data ? (
            ...
            <ul>
              {owner && (
                ...
                >
                  <Avatar size="sm" name={owner.fullName ?? owner.email} email={owner.email} />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm" style={{ color: "var(--text)" }}>
                      {owner.fullName || "Account owner"}
                    </div>
                    <div className="truncate text-xs" style={{ color: "var(--text-muted)" }}>{owner.email}</div>
                  </div>
              ...
              {team.length === 0 && !owner ? (
                ...
                team.map((u) => (
                  <li
                    ...
                    style={{ borderColor: "var(--border)" }}
                  >
                    <Avatar size="sm" name={u.fullName ?? u.email} email={u.email} />
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm" style={{ color: "var(--text)" }}>
                      ...
                      <div className="truncate text-xs" style={{ color: "var(--text-muted)" }}>
                        {u.email}
                        {u.status === "invited" && <span className="ml-1.5" style={{ color: "var(--text-faint)" }}>· pending</span>}
                      </div>
                    </div>
                    <span className="text-xs" style={{ color: "var(--text-muted)" }}>{ROLE_LABEL[u.role]}</span>
                    <button
                      type="button"
          ...
          {teamFetch.error && (
            <p className="mt-2 text-xs" style={{ color: "var(--risk)" }}>{teamFetch.error}</p>
          )}
        </SettingsSection>
          ...
          {whiteLabel && (
            <>
              ...
              <EditableField
                ...
                onSave={async (v) => { setBrandDisplayName(v); }}
              ...
              <EditableField
                ...
                onSave={async (v) => { setBrandSupportEmail(v); }}
        ...
        <SettingsSection icon={<Wallet size={16} />} title="Payout details">
          <EditableField
            ...
            onSave={async (v) => {
              ...
              if (!res.ok) {
                const body = (await res.json().catch(() => ({}))) as { error?: string };
                setNote(body.error === "invalid_email"
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            <Button variant="ghost" onClick={() => setInviteOpen(false)} disabled={inviteBusy}>Cancel</Button>
            <Button onClick={addMember} iconLeft={<Mail size={14} />} disabled={inviteBusy}>
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <Input label="Email" type="email" value={iEmail} onChange={(e) => setIEmail(e.target.value)} placeholder="sam@partner.com" required />
          ...
          {inviteError && (
            <p className="text-xs" style={{ color: "var(--risk)" }}>{inviteError}</p>
          )}
        </div>
      </Modal>
    </TabBody>
...
function humanizeTeamError(code: string | undefined): string {
  switch (code) {
    ...
    case "is_owner":               return "You're already the account owner.";
    case "already_in_team":        return "That email is already on the team.";
    case "user_in_other_reseller": return "That user already belongs to another partner.";
    case "not_signed_in":          return "Your session expired. Sign in again.";
    case "forbidden":              return "You don't have permission to add team members.";
    default:                       return code ? `Couldn't add member (${code}).` : "Couldn't add member.";
  }
}
