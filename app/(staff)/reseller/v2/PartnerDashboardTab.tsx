...
type Dashboard = {
  reseller: { name: string; resellerCode: string; commission: number; status: string; totalEnterprises: number; activeEnterprises: number };
  ...
  return (
    <TabBody>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        <StatCard icon={<Building2 size={16} />} value={num(r?.totalEnterprises)} label="Companies" hint={`${num(r?.activeEnterprises)} active`} />
        <StatCard icon={<Euro size={16} />} value={eur(totalSpend)} label="Portfolio spend" />
        <StatCard icon={<Percent size={16} />} value={`${num(r?.commission)}%`} label="Commission rate" />
        <StatCard icon={<Wallet size={16} />} value={eur(totalCommission)} label="Commission (est.)" />
      </div>
      ...
      <section className="mt-6 rounded-2xl border" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
        <header className="flex items-center justify-between border-b px-4 py-3" style={{ borderColor: "var(--border)" }}>
          <h2 className="text-sm font-semibold" style={{ color: "var(--text)" }}>Companies by spend</h2>
          <Euro size={14} style={{ color: "var(--text-muted)" }} />
        </header>
        {ents.length === 0 ? (
          <div className="p-6"><EmptyState compact title="No companies yet" body="Onboard a company from the Clients tab." /></div>
        ) : (
          <ul>
            {[...ents].sort((a, b) => b.usedMinutes - a.usedMinutes).map((e) => (
              <li key={e.id} className="flex items-center gap-3 border-t px-4 py-3 first:border-t-0" style={{ borderColor: "var(--border)" }}>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs" style={{ color: "var(--text-muted)" }}>
                    since {new Date(e.createdAt).toLocaleDateString()} · commission {eur(Math.round(spend(e.usedMinutes) * commissionPct))}
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm tabular-nums" style={{ color: "var(--text)" }}>{eur(spend(e.usedMinutes))}</div>
