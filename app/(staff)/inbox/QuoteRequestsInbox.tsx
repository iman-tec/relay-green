...
const CATEGORY_META: Record<
  Category,
  { label: string; fill: string; fgVar: string; bgTint: string }
> = {
  appointment: {
    label: "Appointment",
...
export function QuoteRequestsInbox() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch("/api/staff/quote-requests", {
        cache: "no-store",
      });
      if (res.ok)
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                        ...
                        <button
                          ...
                          aria-label={
                            expandedNotes.has(r.id)
                              ? "Collapse customer note"
                              : "Expand customer note"
                          }
                          ...
                          style={{
                            color: "var(--primary-hover)",
                            background:
                              "color-mix(in srgb, var(--primary-hover) 7%, transparent)",
                          }}
                        >
                          ...
                          <ChevronDown
                            ...
                            style={{
                              transform: expandedNotes.has(r.id)
                                ? "rotate(180deg)"
                                : "none",
                            }}
                          />
                    ...
                    {cat === "appointment" &&
                      ...
                      expandedNotes.has(r.id) && (
                        <p
                          className="mt-1.5 rounded-md px-2 py-1.5 text-xs break-words whitespace-pre-wrap"
                          style={{
                            background:
                              "color-mix(in srgb, var(--primary-hover) 6%, transparent)",
                            color: "var(--text)",
                          }}
                        >
                          “{r.appointmentNote}”
...
function BidPrepModal({
  ...
  const readOnly =
    cat === "appointment" || cat === "bid_sent" || cat === "accepted";
  const [amount, setAmount] = useState(
    req.amountCents != null ? String(req.amountCents / 100) : ""
  );
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        className="fixed top-1/2 left-1/2 z-[var(--z-modal)] flex max-h-[90vh] w-full max-w-2xl -translate-x-1/2 -translate-y-1/2 flex-col gap-4 overflow-y-auto rounded-2xl border p-5 shadow-2xl"
        style={{
          borderColor: "var(--border)",
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <h2
              ...
              style={{ color: "var(--text)" }}
            >
              {readOnly ? "View bid" : "Bid"} ·{" "}
              {req.kind === "golive" ? "Go-live" : "Maintain"} — {req.project}
            </h2>
        ...
        {req.appointmentRequestedAt && (
          ...
          >
            <CalendarClock size={12} className="mr-1 inline" /> Customer
            requested an appointment
            {req.appointmentNote ? `: "${req.appointmentNote}"` : "."}
          </div>
        )}
        ...
        >
          <div
            ...
            style={{ color: "var(--text-muted)" }}
          >
            <FileText size={12} /> {readOnly ? "Bid (read-only)" : "Bid"}
          </div>
          <label
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label
              ...
              style={{ color: "var(--text-muted)" }}
            >
              {readOnly ? "Valid until" : "Valid for (days)"}
              {readOnly ? (
                <input
                  value={
                    req.bidValidityUntil
                      ? new Date(req.bidValidityUntil).toLocaleDateString()
                      : "—"
                  }
                  disabled
                  className="h-10 rounded-lg border px-3 text-sm disabled:opacity-70"
