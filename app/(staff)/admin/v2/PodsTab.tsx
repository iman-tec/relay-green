...
export function PodsTab() {
  ...
  const [statusByUser, setStatusByUser] = useState<Map<string, MemberAuthStatus>>(new Map());
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const body = (await res.json().catch(() => ({}))) as { pods?: Pod[]; error?: string };
      if (!res.ok || !body.pods) {
  ...
  useEffect(() => {
    if (!selPod) return;
    const memberIds = [...selPod.supervisors, ...selPod.engineers].map((m) => m.userId);
  ...
  const archivePod = async (id: string) => {
    if (!confirm("Archive this pod? Members will stay assigned until you remove them.")) return;
    ...
    if (res.ok) {
      setPodId(null);
      refresh();
    } else alert((await res.json().catch(() => ({}))).error ?? "Archive failed.");
  };

  const removeMember = async (userId: string) => {
    if (!selPodId) return;
    if (!confirm("Remove this user from the pod?")) return;
    const res = await fetch(`/api/admin/pods/${selPodId}/members/${userId}`, { method: "DELETE" });
    if (res.ok) refresh();
  ...
  const toggleMemberStatus = async (userId: string, currentlyActive: boolean) => {
  ...
  return (
    <div className="flex h-full min-h-0">
        ...
        renderRow={(it) => {
          ...
          return (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-1.5 text-[10px] tracking-wider uppercase" style={{ color: "var(--text-muted)" }}>
      ...
      <PickPodMemberDrawer
        ...
        onAdded={() => { setAddEngineer(false); refresh(); }}
      />
      <PickPodMemberDrawer
        open={addSupervisor}
        podId={selPodId}
        ...
        onAdded={() => { setAddSupervisor(false); refresh(); }}
      />
    </div>
  );
}
...
function MemberSection({
  title, role, members, statusByUser,
            ...
            return (
              ...
              >
                ...
                <span
                  ...
                  style={{
                    color: status === "active" ? "#3dcb7e" : "var(--text-muted)",
                ...
                <div className="flex items-center gap-1">
                  <RowIcon title="Resend invite email" onClick={() => onResend(m.userId)}>
                    <Mail className="size-3.5" />
                  </RowIcon>
                  <RowIcon
                    title={status === "active" ? "Deactivate" : "Reactivate"}
                    onClick={() => onToggleStatus(m.userId, status === "active")}
                  >
                    {status === "active" ? <PowerOff className="size-3.5" /> : <Power className="size-3.5" />}
                  </RowIcon>
                  <RowIcon title={`Remove ${role} from pod`} danger onClick={() => onRemove(m.userId)}>
