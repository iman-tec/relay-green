...
/*
 * Add-employee drawer. POSTs to
 * /api/admin/orgs/:orgId/departments/:deptId/employees — invites an
 * employee, links them to the dept, transfers initial minutes from the
 * dept pool to the employee pool.
 */
...
export function AddEmployeeDrawer({
  ...
  const submit = async () => {
    if (!orgId || !deptId) {
      setError("Pick an enterprise + department first.");
      return;
    }
