 ...
 * Two paths in one drawer — promote an existing dept employee, or invite a
 * brand-new admin by email. POSTs to
 * /api/admin/orgs/:orgId/departments/:deptId/admin (fills an empty slot only).
 *
 * Admin-side twin of app/(staff)/enterprise/v2/_drawers/AssignAdminDrawer.tsx.
 */
...
export function AssignAdminDrawer({
  ...
  const submit = async () => {
    if (!orgId || !deptId) { setError("Pick a department first."); return; }
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/orgs/${orgId}/departments/${deptId}/admin`, {
        method:  "POST",
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex gap-1.5">
          <ModeChip active={mode === "promote"} disabled={!hasEmployees} onClick={() => setMode("promote")}>
            Promote employee
          </ModeChip>
...
function humanise(code: string | undefined): string {
  switch (code) {
    ...
    case "not_owned":                return "Department not found in this organisation.";
