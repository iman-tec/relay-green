...
export function AdminClient() {
  ...
  useEffect(() => {
    ...
    const ch = sb
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "session_audit_log" },
        () => {
          void refresh();
        }
      )
  ...
  return (
    <div className="mx-auto max-w-screen-2xl space-y-4 px-6 py-6">
      ...
      <Section
        title="Activity"
        subtitle="Daily sessions over the last 30 days."
      >
        <div className="p-5">
          {loading ? (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Section
          title="Users & roles"
          subtitle="Click a role to grant. Click again to revoke (engineer only)."
        >
          {loading ? (
            <Loading />
          ) : (
        ...
        <Section
          title="Audit log"
          subtitle="Latest 40 events across all sessions."
        >
          {loading ? (
            <Loading />
          ) : audit.length === 0 ? (
            ...
            <div className="max-h-[480px] overflow-y-auto">
              {audit.map((a) => (
                <AuditRowDisplay key={a.id} row={a} />
              ))}
            </div>
          )}
        </Section>
    ...
    >
      ...
      {points.map((p, i) => (
        <g key={i}>
          ...
          {p.count > 0 && (
            <circle cx={p.x} cy={p.y} r="2.4" fill={BRAND_GREEN} />
          )}
        </g>
      ))}
    </svg>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div
                  ...
                  style={{ color: "var(--text-muted)" }}
                >
                  {formatRole(p.primary_role) || "no primary"}
                </div>
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="ml-5 text-[10px]" style={{ color: "var(--text-muted)" }}>
        {new Date(row.created_at).toLocaleString()} · session{" "}
