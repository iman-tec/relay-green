      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex items-baseline justify-between gap-4">
          <div>
            ...
            >
              Signed in as{" "}
              <span style={{ color: "var(--text)" }}>{meEmail}</span>
            </p>
          </div>
        </header>
...
function StaffTab({ meEmail }: { meEmail: string }) {
  ...
  const saveRole = async (id: string, role: RoleKey) => {
    ...
    try {
      ...
      if (!res.ok) {
        setActionError(body.error ?? "Role update failed.");
        return;
      }
  ...
  const ensurePodsLoaded = async () => {
    ...
    try {
      ...
      /* silent — pod field falls back to "None" */
    } finally {
      setPodsLoaded(true);
    }
  };
  ...
  const submitAdd = async () => {
    ...
    try {
      ...
      if (podId === NEW_POD_KEY) {
        ...
        const podBody = (await podRes.json().catch(() => ({}))) as {
      ...
      const podRoleFor: "engineer" | "supervisor" | null =
        draft.role === ROLE.engineer
          ? "engineer"
          : draft.role === ROLE.supervisor
            ? "supervisor"
            : null;
      const podName = podId
      ...
      if (podId && podRoleFor) {
        ...
        if (!assignRes.ok) {
          setActionError(
            `User invited, but pod assignment failed: ${assignBody.error ?? "unknown error"}`
          );
        }
      }
      ...
      setActionError(
        err instanceof Error ? err.message : "Couldn't create user."
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const resendInvite = async (row: UserRow) => {
    ...
    try {
      ...
      const body = (await res.json().catch(() => ({}))) as { error?: string };
      if (!res.ok) {
        setActionError(body.error ?? "Resend failed.");
        return;
      }
      ...
      render: (r) => {
        ...
        return (
          <div className="inline-flex items-center gap-1">
            ...
            <IconAction
              onClick={() => {
                if (!isSuper) void deleteRow(r);
              }}
              title={
                isSuper ? "Super Admins can't be deleted from here" : "Delete"
              }
              icon={<Trash2 size={14} />}
              danger
              disabled={isSuper}
  ...
  return (
    <div className="flex flex-col gap-3">
      {actionError && (
        <ErrorBanner
          message={actionError}
          dismiss={() => setActionError(null)}
        />
      )}
      {actionInfo && (
        <SuccessToast
          message={actionInfo}
          dismiss={() => setActionInfo(null)}
        />
      )}
...
function DraftForm({
  ...
  // pod_members.
  const podEligible =
    draft.role === ROLE.engineer || draft.role === ROLE.supervisor;
      ...
      <div className="grid gap-2 md:grid-cols-[1.4fr_1.4fr_1fr_1fr]">
        ...
        <select
          ...
          title={
            podEligible
              ? "Assign to a pod"
              : "Pods only apply to engineers and supervisors"
          }
        >
          ...
          {podEligible && (
            <option
              value={NEW_POD_KEY}
              style={{ backgroundColor: "var(--surface)", color: BRAND_GREEN }}
            >
              + Create new pod…
            </option>
          )}
