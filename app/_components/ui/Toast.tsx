...
// Border tints used to be 40-45% mix of the tone color — visibly loud on
// the white canvas next to the soft-fill bg. Dropped to 22% (~hairline)
// so the toast reads as a tint of the canvas instead of a coloured card.
const TONE_RING: Record<StatusTone, string> = {
  ok: "border-[color-mix(in_srgb,var(--ok)_22%,transparent)] bg-[var(--ok-soft)]",
  warn: "border-[color-mix(in_srgb,var(--warn)_22%,transparent)] bg-[var(--warn-soft)]",
  risk: "border-[color-mix(in_srgb,var(--risk)_28%,transparent)] bg-[var(--risk-soft)]",
  info: "border-[color-mix(in_srgb,var(--primary)_22%,transparent)] bg-[var(--primary-soft)]",
  neutral:
