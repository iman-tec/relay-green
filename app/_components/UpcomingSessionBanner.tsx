...
export function UpcomingSessionBanner() {
  ...
  const load = useCallback(async (uid: string) => {
    ...
    const rows = (data ?? []) as Array<{ id: string; engineer_user_id: string; project_id: string | null; slot_start: string }>;
    ...
    setItems(rows.map((r) => ({
      ...
      projectName: (r.project_id && pname.get(r.project_id)) || "your project",
    })));
  }, []);
  ...
  const next = items.find((i) => new Date(i.slotStart).getTime() > nowMs - 60_000) ?? null;
  if (!next) return null;

  return (
    <div className="pointer-events-none fixed left-1/2 top-3 z-40 -translate-x-1/2 px-4">
      <div
      ...
      >
        ...
        <span className="text-[12px]" style={{ color: "var(--text-muted)" }}>
          Scheduled session ·{" "}
          <span style={{ color: "var(--text)", fontWeight: 600 }}>{next.projectName}</span>
          {" · "}
