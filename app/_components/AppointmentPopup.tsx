  ...
  const active = useMemo(() => {
    return (
      appts.find((a) => {
        ...
        return (
          nowMs >= start - JOIN_LEAD_MIN * 60_000 &&
          nowMs <= end + GRACE_MIN * 60_000
        );
      }) ?? null
    );
  ...
  const join = useCallback(
    async (a: Appt) => {
      ...
      try {
        ...
        const { data, error: rpcErr } = await sb.rpc("launch_booked_session", {
          _booking_id: a.id,
        });
        if (rpcErr) {
          throw new Error(
            /NO_ENTITLEMENT/.test(rpcErr.message)
    ...
    async (a: Appt) => {
      ...
      try {
        ...
        if (reason === "later") {
          ...
          setReschedule({
            engineerUserId: a.engineerUserId,
            engineerName: a.engineerName,
            projectId: a.projectId,
          });
        } else {
          const label =
            CANCEL_REASONS.find((r) => r.key === reason)?.label ?? reason;
          await sb.rpc("cancel_booking_with_reason", {
            _id: a.id,
            _reason: label,
          });
  ...
  if (reschedule) {
    return (
      <ScheduleEngineerModal
        ...
        onBooked={() => {
          /* modal shows its own confirmation */
        }}
      />
    );
  }
        ...
        {!cancelOpen ? (
          <div className="mt-4 flex gap-2">
            ...
            <button
              ...
              className="rounded-full border px-4 py-2.5 text-sm font-medium transition-colors hover:bg-black/5 disabled:opacity-50 dark:hover:bg-white/5"
              style={{
                borderColor: "var(--border)",
          ...
          <div className="mt-4">
            ...
            <div className="space-y-1">
              {CANCEL_REASONS.map((r) => (
                <button
                  ...
                  style={{
                    borderColor:
                      reason === r.key ? "var(--primary)" : "var(--border)",
                    backgroundColor:
                      reason === r.key ? "var(--primary-soft)" : "transparent",
                    color: "var(--text)",
                  }}
                >
                  <span
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                style={{
                  backgroundColor:
                    reason === "later" ? "var(--primary)" : "var(--accent-red)",
                }}
              >
                {busy ? (
