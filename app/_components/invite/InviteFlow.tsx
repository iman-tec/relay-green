...
export function InviteFlow({
  ...
  const companyParsed = bulkOnly
    ...
    : {
        recipients: [] as ParsedCompaniesRecipient[],
        missingColumns: [] as string[],
        rowErrors: [] as string[],
        rowWarnings: [] as string[],
      };
  ...
  const send = async () => {
    if (variant === "companies") {
      if (!canSend) {
        setErr("Fix the CSV before sending.");
        return;
      }
      setBusy(true);
      setErr(null);
        ...
        for (const r of companyParsed.recipients) {
          try {
            ...
            out.push({
              email: r.email,
              ok: res.ok,
              error: res.ok ? undefined : b.error || "failed",
            });
          } catch (e) {
            out.push({
              email: r.email,
    ...
    try {
      ...
      finalize(
        b.results ?? recipients.map((x) => ({ email: x.email, ok: true }))
      );
    } catch (e) {
  ...
  const heading =
    title ?? (variant === "companies" ? "Add companies" : "Invite people");
  const sentCount = results?.filter((r) => r.ok).length ?? 0;

  const companyMissingMsg =
    companyParsed.missingColumns.length > 0
  ...
  return (
    <Modal
      open={open}
      onClose={reset}
      title={results ? "Invites sent" : heading}
      ...
      {results ? (
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-1.5">
              {(["single", "bulk"] as const).map((m) => (
                <button
                  ...
                  style={{
                    borderColor:
                      mode === m ? "var(--primary)" : "var(--border)",
                    background:
                      mode === m ? "var(--primary-tint)" : "transparent",
                    color:
                      mode === m ? "var(--primary-hover)" : "var(--text-muted)",
                  }}
                >
          ...
          {!bulkOnly && mode === "single" ? (
            ...
            <>
              <label
                className="inline-flex w-fit cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm"
              ...
              {bulkOnly ? (
                ...
                <div
                  ...
                  style={{ color: "var(--text-muted)" }}
                >
                  <Users size={13} /> {memberParsed.recipients.length} valid
                  recipient{memberParsed.recipients.length === 1 ? "" : "s"}
                  {memberParsed.errors.length > 0 ? (
                    <span style={{ color: "var(--warn)" }}>
                      · {memberParsed.errors.length} skipped
                    </span>
                  ) : null}
                </div>
              )}
    ...
    <div className="flex flex-col gap-2">
      {rowErrors.length > 0 && (
        ...
        >
          ...
          >
            ...
            {rowErrors.length} row{rowErrors.length === 1 ? "" : "s"} need
            fixing
          </div>
          <ul
      ...
      {rowWarnings.length > 0 && (
        ...
        >
          ...
          <ul
            ...
            style={{ color: "var(--text-muted)" }}
          >
            {rowWarnings.slice(0, 8).map((msg, i) => (
              <li key={i}>{msg}</li>
            ))}
            {rowWarnings.length > 8 && (
              <li style={{ color: "var(--text-faint)" }}>
                … and {rowWarnings.length - 8} more.
              </li>
            )}
          </ul>
        </div>
      ...
      {rowErrors.length === 0 && (
        <div
          ...
          style={{ color: "var(--text-muted)" }}
        >
          <Users size={13} /> {validCount} valid recipient
          {validCount === 1 ? "" : "s"}
        </div>
      )}
    </div>
...
export function inviteHint(): ReactNode {
  return null;
}
