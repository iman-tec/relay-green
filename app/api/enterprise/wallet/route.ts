...
import {
  MINUTE_BUNDLES,
  LIST_CENTS_PER_MINUTE,
} from "@/lib/billing/minuteBundles";
...
export async function GET() {
  ...
  const { data: org, error } = await admin
    .from("organizations")
    .select(
      "allocated_minutes, used_minutes, remaining_minutes, billing_currency"
    )
    .eq("id", orgId)
    .single();
  if (error || !org) {
  ...
  const distributedMinutes = (
    (depts ?? []) as Array<{ allocated_minutes: number }>
