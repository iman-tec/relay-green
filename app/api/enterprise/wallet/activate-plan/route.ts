...
export async function POST(request: Request) {
  ...
  const { tier, paymentIntentId } = (await request
    .json()
    .catch(() => ({}))) as {
    tier?: PlanTier;
  ...
  if (!tier || !VALID_TIERS.has(tier)) {
    return NextResponse.json(
      { error: "Need tier ∈ starter/pro/business/enterprise." },
      { status: 400 }
    );
  }
  ...
  if (planDef.monthlyPriceCents == null && tier === "enterprise") {
    ...
    return NextResponse.json(
      {
        error:
          "Enterprise tier is contact-sales — reach out to your account manager.",
      },
      { status: 400 }
    );
  }
