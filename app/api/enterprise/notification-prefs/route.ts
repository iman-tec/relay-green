...
export async function PUT(request: Request) {
  ...
  const merged = {
    ...
    session_alerts:
      body.sessionAlerts ?? existing?.session_alerts ?? DEFAULTS.sessionAlerts,
    low_minutes:
      body.lowMinutes ?? existing?.low_minutes ?? DEFAULTS.lowMinutes,
    weekly_digest:
      body.weeklyDigest ?? existing?.weekly_digest ?? DEFAULTS.weeklyDigest,
  };
