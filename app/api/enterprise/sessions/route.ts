...
export async function GET(request: Request) {
  ...
  let q = admin
    .from("guest_calls")
    .select(
      "id,status,urgency,recall_count,created_at,joined_at,ended_at,duration_minutes,customer_user_id,claimed_by,guest_name,project_name"
    )
    .or(orFilter)
  ...
  const sessions = rows.map((r) => ({
    ...
    chargeCents: r.duration_minutes
      ? Math.round(Number(r.duration_minutes) * CENTS_PER_MINUTE)
      : null,
