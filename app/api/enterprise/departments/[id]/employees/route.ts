...
export async function GET(_request: Request, { params }: RouteCtx) {
  ...
  if (!dept || dept.enterprise_id !== orgId) {
    return NextResponse.json(
      { error: "Department not found in your org." },
      { status: 404 }
    );
  }
  ...
  const roleIds = Array.from(
    new Set(
      profiles.map((p) => p.primary_role_id).filter((x): x is string => !!x)
    )
  );
...
export async function POST(request: Request, { params }: RouteCtx) {
  ...
  if (!userId) {
    return NextResponse.json(
      { error: "Employee invited but auth row not visible. Try again." },
      { status: 500 }
    );
  }
  ...
  if (allocNum > 0) {
    ...
    if (tErr)
      console.warn(
        "[enterprise/departments/employees] initial transfer failed:",
        tErr.message
      );
  }
