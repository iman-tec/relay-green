...
export async function GET(request: Request) {
  ...
  if (scope !== "staff" && scope !== "users") {
    return NextResponse.json(
      { error: "scope must be 'staff' or 'users'." },
      { status: 400 }
    );
  }
  ...
  const members = list
    .map((p) => {
      ...
      const isStaff = Array.from(rolesForUser).some((r) =>
        STAFF_ROLE_SET.has(r)
      );
...
async function provisionMember(
  ...
  const existing0 = lookup0.data?.users?.find(
  ...
  if (existing0) {
    const { data: prior } = await admin
      ...
      .eq("id", existing0.id)
      .maybeSingle();
    const otherOrg = (prior as { organization_id: string | null } | null)
      ?.organization_id;
    if (otherOrg && otherOrg !== orgId) {
      return {
        ...
        status: 409,
        error:
          "This user already belongs to another organization. Ask a super admin to release them first.",
      };
    }
  }
  ...
  const isEmployeeInvite =
    role === ROLE.client && resolvedDepartmentId !== null;
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => ({}))) as ProvisionInput & {
    recipients?: Array<{
      email?: string;
      name?: string;
      displayName?: string;
  ...
  if (Array.isArray(body.recipients)) {
    ...
    for (const rec of recipients) {
      ...
      if (res.ok) {
        await track(email, name, role, res.departmentId);
