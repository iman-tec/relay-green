...
export async function POST() {
  ...
  type OrgRow = {
    ...
    enterprise_code: string; created_at: string;
    plan_tier: string | null; plan_status: string | null;
    retention_days: number | null; reseller_id: string | null;
  };
  const { data: orgRow, error: orgErr } = await admin
    .from("organizations")
    .select("id, name, primary_domain, status, enterprise_code, created_at, plan_tier, plan_status, retention_days, reseller_id")
    .eq("id", orgId)
    .single<OrgRow>();
  if (orgErr || !orgRow) {
  ...
  // ---- Departments ----
  type DeptRow = { id: string; name: string; status: string; created_at: string };
  ...
  type ProfileRow = {
    id: string; full_name: string | null; status: string | null;
    department_id: string | null; created_at: string; erased_at: string | null;
  };
  ...
  const usageByMember = new Map<string, { sessions: number; minutes: number; spendCents: number }>();
  for (const s of sessions) {
    if (!s.customer_user_id) continue;
    const u = usageByMember.get(s.customer_user_id) ?? { sessions: 0, minutes: 0, spendCents: 0 };
    u.sessions += 1;
  ...
  const orgCsv = toCsv(
    ...
    [[orgRow.id, orgRow.name, orgRow.primary_domain ?? "", orgRow.status, orgRow.enterprise_code, orgRow.created_at, orgRow.plan_tier ?? "", orgRow.plan_status ?? "", orgRow.retention_days ?? "", orgRow.reseller_id ?? ""]],
  );
  ...
  const membersCsv = toCsv(
    ["id", "email", "full_name", "status", "department_id", "created_at", "erased_at"],
    profiles.map((p) => [
      p.id,
      emailById.get(p.id) ?? "",
  ...
  const sessionsCsv = toCsv(
    ...
    sessions.map((s) => [
      s.id, s.status, s.created_at, s.duration_minutes ?? 0,
      s.customer_user_id ?? "", s.organization_id ?? "",
    ]),
  );
  ...
  const safeName = orgRow.name.replace(/[^a-z0-9-_]+/gi, "_").replace(/^_+|_+$/g, "") || "organization";
