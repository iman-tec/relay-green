...
export async function GET() {
  ...
  const { data: rRow } = await admin
    .from("resellers")
    .select(
      "id, name, reseller_code, status, commission, allocated_minutes, used_minutes, remaining_minutes"
    )
    .eq("id", resellerId)
    .maybeSingle();
  ...
  if (orgIds.length > 0) {
    ...
    for (const d of (deptRows ?? []) as { enterprise_id: string }[]) {
      departmentCountByOrg.set(
        d.enterprise_id,
        (departmentCountByOrg.get(d.enterprise_id) ?? 0) + 1
      );
    }
  }
