...
export async function PUT(request: Request) {
  ...
  const merged = {
    ...
    new_client_onboarded:
      body.newClientOnboarded ??
      existing?.new_client_onboarded ??
      DEFAULTS.newClientOnboarded,
    client_low_minutes:
      body.clientLowMinutes ??
      existing?.client_low_minutes ??
      DEFAULTS.clientLowMinutes,
    payout_processed:
      body.payoutProcessed ??
      existing?.payout_processed ??
      DEFAULTS.payoutProcessed,
  };
