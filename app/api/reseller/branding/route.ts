...
export async function PUT(request: Request) {
  ...
  if (body.supportEmail != null && body.supportEmail !== "" && !EMAIL.test(body.supportEmail)) {
    return NextResponse.json({ error: "invalid_support_email" }, { status: 400 });
  }
  ...
  const merged = {
    ...
    white_label_enabled: body.whiteLabelEnabled ?? existing?.white_label_enabled ?? DEFAULTS.whiteLabelEnabled,
    accent_color:        body.accentColor       ?? existing?.accent_color        ?? DEFAULTS.accentColor,
    display_name:        body.displayName       ?? existing?.display_name        ?? DEFAULTS.displayName,
    support_email:       body.supportEmail      ?? existing?.support_email       ?? DEFAULTS.supportEmail,
  };
