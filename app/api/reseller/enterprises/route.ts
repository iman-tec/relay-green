...
export async function POST(request: Request) {
  ...
  const { data: orgRow, error: orgErr } = await admin
    ...
    .insert(orgInsert)
    .select(
      "id, name, primary_domain, status, enterprise_code, enterprise_type, reseller_id, created_at"
    )
    .single();
  if (orgErr || !orgRow) {
    return NextResponse.json(
      { error: orgErr?.message ?? "Couldn't create enterprise." },
      { status: 400 }
    );
  }
  const org = orgRow as {
  ...
  if (allocNum > 0) {
    ...
    if (tErr) {
      ...
      console.warn(
        "[reseller/enterprises] initial transfer failed:",
        tErr.message
      );
    }
  }
