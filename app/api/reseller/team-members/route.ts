...
const ALLOWED_ROLES: ReadonlySet<RoleLabel> = new Set([
  "manager",
  "analyst",
  "admin",
]);
...
export async function GET() {
  ...
  const [{ data: reseller, error: rErr }, { data: members, error: mErr }] =
    await Promise.all([
      admin
        .from("resellers")
      ...
      admin
        ...
        .select(
          "id, email, full_name, role, status, user_id, invited_at, accepted_at"
        )
  ...
  let owner: { id: string; email: string; fullName: string | null } | null =
    null;
  if (reseller?.owner_user_id) {
    const { data: ownerUser } = await admin.auth.admin.getUserById(
      reseller.owner_user_id
    );
...
export async function POST(request: Request) {
  ...
  if (existingUserId) {
    ...
    if (
      existingProfile?.reseller_id &&
      existingProfile.reseller_id !== resellerId
    ) {
      return NextResponse.json(
        { error: "user_in_other_reseller" },
        { status: 409 }
      );
    }
  }
