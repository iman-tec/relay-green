...
> {
  ...
  const [{ data: roles }, { data: profile }] = await Promise.all([
    ...
    supabase.from("profiles").select("organization_id, department_id, reseller_id").eq("id", user.id).maybeSingle(),
  ]);
  const roleSet = new Set((roles ?? []).map((r: { role: string }) => r.role));
  const p = (profile ?? {}) as { organization_id?: string; department_id?: string; reseller_id?: string };

  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) return { ok: false, status: 500, error: "service_role_not_configured" };
  ...
  if (roleSet.has(ROLE.reseller)) {
    ...
    if (rid) scope = { type: "partner", id: rid };
      ...
      const { data: r } = await admin.from("resellers").select("id").eq("owner_user_id", user.id).maybeSingle();
...
export async function GET() {
  ...
  const { data, error } = await r.admin
    .from("invites")
    .select("id, email, name, role, company_name, status, sent_at, opened_at, accepted_at, expires_at")
...
type Recipient = { email?: string; name?: string; role?: string; departmentId?: string };
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => ({}))) as { recipients?: Recipient[] };
  const recipients = (body.recipients ?? []).filter((x) => x.email && x.email.includes("@"));
  ...
  const results: Array<{ email: string; ok: boolean; error?: string; link?: string }> = [];
  for (const rec of recipients) {
    const email = rec.email!.trim().toLowerCase();
    try {
      // Default role by scope.
      const role = rec.role
        || (scope.type === "company" || scope.type === "department" ? ROLE.client : "partner_member");
      const inv = await recordInvite(admin, {
        email, name: rec.name ?? null, role,
        scopeType: scope.type, scopeId: scope.id,
        departmentId: rec.departmentId ?? null,
        invitedBy: userId,
      });
      if ("error" in inv) { results.push({ email, ok: false, error: inv.error }); continue; }
      ...
      await sendInvitationEmail(admin, {
        ...
        metadata: { invite_role: role, invite_scope_type: scope.type, invite_scope_id: scope.id, invite_code: inv.row.code },
      });
