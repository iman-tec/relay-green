...
export async function GET() {
  ...
  if (engineerIds.length === 0) return NextResponse.json({ state: "insufficient", sampleSize: 0, themes: [] });
  ...
  const { data: escs } = await admin
    ...
    .limit(200);
  const rows = (escs ?? []) as { reason: string; note: string | null; resolution_note: string | null }[];

  if (rows.length < MIN_FOR_THEMES) {
    return NextResponse.json({ state: "insufficient", sampleSize: rows.length, themes: [] });
  }

  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return NextResponse.json({ state: "unavailable", sampleSize: rows.length, themes: [] });

  const lines = rows.map((r, i) => `${i + 1}. reason="${r.reason}"${r.note ? ` note="${r.note}"` : ""}${r.resolution_note ? ` resolution="${r.resolution_note}"` : ""}`).join("\n");
  try {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: 'You cluster support-escalation notes into recurring themes. Return JSON: {"themes":[{"theme":"short label","count":N}]}. Only themes with count>=2, sorted by count desc, max 6. Theme labels are 1-4 words (e.g. "Stripe payments", "Zoom join failures").' },
          { role: "user", content: `Escalations (last 30 days):\n${lines}` },
        ],
      }),
    });
    if (!res.ok) return NextResponse.json({ state: "unavailable", sampleSize: rows.length, themes: [] });
    const json = (await res.json()) as { choices?: { message?: { content?: string } }[] };
    const parsed = JSON.parse(json.choices?.[0]?.message?.content ?? "{}") as { themes?: { theme: string; count: number }[] };
    const themes = (parsed.themes ?? []).filter((t) => t.theme && t.count >= 2).slice(0, 6);
    return NextResponse.json({ state: "ok", sampleSize: rows.length, themes });
  } catch {
    return NextResponse.json({ state: "unavailable", sampleSize: rows.length, themes: [] });
  }
}
