...
export async function GET() {
  ...
  if (engineerIds.length === 0) return NextResponse.json({ total: { earningsCents: 0, billableMinutes: 0, sessions: 0 }, engineers: [] });

  const [{ data: earn }, { data: profs }] = await Promise.all([
    admin.from("engineer_earnings_summary").select("engineer_user_id, total_sessions, billable_minutes, lifetime_earnings_cents, most_recent_session_at").in("engineer_user_id", engineerIds),
  ...
  type E = { engineer_user_id: string; total_sessions: number; billable_minutes: number; lifetime_earnings_cents: number | null; most_recent_session_at: string | null };
  const rows = (earn ?? []) as E[];
  const engineers = rows.map((e) => ({
  ...
  const total = engineers.reduce((t, e) => ({
