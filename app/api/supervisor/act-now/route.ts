...
export async function GET() {
  ...
  const [{ data: quotes }, { data: callbacks }, { data: escalations }] =
    await Promise.all([
      admin
        .from("project_quote_requests")
        .select(
          "id, kind, comments, status, created_at, project_id, customer_user_id, quote_amount_cents, bid_scope, bid_timeline, appointment_requested_at, appointment_note, customer_response_note, responded_at"
        )
        .in("status", [
          "pending",
          "pending_review",
      ...
      podEngineerIds.length
        ? admin
            ...
            .select(
              "id, customer_user_id, engineer_user_id, project_id, message, status, created_at, expires_at"
            )
            .eq("status", "pending")
        ...
        ? admin
            ...
            .or(
              [
                ...
                podEngineerIds.length
                  ? `engineer_user_id.in.(${podEngineerIds.join(",")})`
                  : "",
              ]
                .filter(Boolean)
                .join(",")
  ...
  for (const q of qs) {
    userIds.add(q.customer_user_id);
    projectIds.add(q.project_id);
  }
  for (const c of cs) {
    userIds.add(c.customer_user_id);
    userIds.add(c.engineer_user_id);
    if (c.project_id) projectIds.add(c.project_id);
  }
  for (const e of es) {
    userIds.add(e.engineer_user_id);
  ...
  if (quoteCustomerIds.length) {
    const { data: liveSess } = await admin
      ...
      .order("created_at", { ascending: false });
    for (const s of (liveSess ?? []) as {
      id: string;
      customer_user_id: string | null;
    }[]) {
      if (s.customer_user_id && !liveByCustomer.has(s.customer_user_id))
        liveByCustomer.set(s.customer_user_id, s.id);
    }
  ...
  return NextResponse.json({
    estimationRequests: qs.map((q) => {
      ...
      return {
        ...
        liveSessionId,
        liveSentiment: liveSessionId
          ? (liveSentiment.get(liveSessionId) ?? null)
          : null,
      };
    }),
