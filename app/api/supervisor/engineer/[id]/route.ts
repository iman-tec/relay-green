...
export async function GET(
  ...
  const { data: membership } = await admin
    ...
    .maybeSingle();
  if (!membership)
    return NextResponse.json({ error: "not_in_pod" }, { status: 403 });
  ...
  const [
    ...
    admin
      ...
      .select("presence_state, is_available, updated_at")
      .eq("user_id", engineerId)
      .maybeSingle(),
    admin
      .from("guest_calls")
      .select(
        "id, guest_name, status, duration_minutes, created_at, ended_at, project_name"
      )
    ...
    admin
      ...
      .select(
        "id, reason, note, status, resolution_note, created_at, resolved_at"
      )
    ...
    admin
      ...
      .lt("slot_start", next14)
      .order("slot_start", { ascending: true })
      .limit(10),
    admin
      .from("user_devices")
      .select("id, device_label, user_agent, last_seen_at")
      .eq("user_id", engineerId)
      .order("last_seen_at", { ascending: false }),
  ]);

  const kpis = (kpiRows ?? []) as {
    duration_minutes: number | null;
    status: string;
    ended_at: string | null;
  }[];
  const buildMinutes = kpis.reduce(
    (s, r) => s + Math.round(Number(r.duration_minutes ?? 0)),
    0
  );
  const ended = kpis.filter((r) => r.ended_at);
  const avgDurationMin = ended.length
    ? Math.round(
        ended.reduce((s, r) => s + Number(r.duration_minutes ?? 0), 0) /
          ended.length
      )
    : 0;
  const pres = profile as {
  ...
  const escalationRate =
    kpis.length > 0 ? Math.round((esc30d / kpis.length) * 10 * 10) / 10 : 0;
  ...
  return NextResponse.json({
    ...
    availability: {
      weekdays: [
        ...new Set(
          ((winRows ?? []) as { weekday: number }[]).map((w) => w.weekday)
        ),
      ].sort(),
      holidays: (
        (holRows ?? []) as {
      ...
      ).map((h) => ({ date: h.holiday_date, label: h.label, kind: h.kind })),
      upcomingBookings: ((bookRows ?? []) as { slot_start: string }[]).map(
        (b) => b.slot_start
      ),
    },
    devices: (
      ...
      id: d.id,
      label: d.device_label ?? "Unknown device",
      lastSeenAt: d.last_seen_at,
    })),
    recentSessions: (recent ?? []).map(
      (r: {
        id: string;
