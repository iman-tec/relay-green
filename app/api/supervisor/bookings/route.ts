...
export async function GET() {
  ...
  const { data: rows } = await admin
    ...
    .select(
      "id, engineer_user_id, customer_user_id, project_id, slot_start, slot_end, status"
    )
  ...
  const userIds = [
    ...new Set(bks.flatMap((b) => [b.engineer_user_id, b.customer_user_id])),
  ];
  const projIds = [
    ...new Set(bks.map((b) => b.project_id).filter(Boolean) as string[]),
  ];
