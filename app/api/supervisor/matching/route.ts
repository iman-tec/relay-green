...
export async function GET() {
  ...
  const podEngineerIds = (members ?? []).map(
  ...
  for (const c of (closedRes.data ?? []) as {
    intake_id: string;
    guest_call_id: string | null;
  }[]) {
    if (
      !pendingIntakeIds.has(c.intake_id) &&
      !strandedCandidates.has(c.intake_id)
    ) {
  ...
  const intakeIds = Array.from(
    new Set([
      ...typedOffers.map((o) => o.intake_id),
      ...strandedCandidates.keys(),
    ])
  );
  ...
  const [intakesRes, callsRes, engProfilesRes] = await Promise.all([
    ...
    guestCallIds.length
      ? admin
          .from("guest_calls")
          .select(
            "id, project_name, guest_name, created_at, status, claimed_by"
          )
          .in("id", guestCallIds)
  ...
  rows.sort((a, b) =>
    a.allDeclined === b.allDeclined ? 0 : a.allDeclined ? -1 : 1
  );
