...
export async function GET() {
  ...
  for (const p of (profs ?? []) as { id: string; full_name: string | null }[])
    nameById.set(p.id, p.full_name ?? "Supervisor");
  const onlineById = new Map<string, boolean>();
  for (const p of (pres ?? []) as { user_id: string; is_online: boolean }[])
    onlineById.set(p.user_id, p.is_online);
  ...
  const supervisors = supIds
    ...
    .sort(
      (a, b) =>
        Number(b.isOnline) - Number(a.isOnline) || a.name.localeCompare(b.name)
    );
