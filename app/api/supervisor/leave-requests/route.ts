...
export async function GET() {
  ...
  const { data: rows } = await admin
    .from("leave_requests")
    .select("id, requester_user_id, start_date, end_date, total_days, reason, kind, status, created_at")
    .eq("pod_id", podId)
    ...
    .limit(200);
  const reqs = (rows ?? []) as {
    id: string; requester_user_id: string; start_date: string; end_date: string;
    total_days: number; reason: string; kind: string; status: string; created_at: string;
  }[];
