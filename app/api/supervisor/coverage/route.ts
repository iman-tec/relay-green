...
export async function GET(req: Request) {
  ...
  const days = Math.min(31, Math.max(1, Number(new URL(req.url).searchParams.get("days") ?? 7)));
  ...
  if (engineerIds.length === 0) {
    return NextResponse.json({ days, openHour: OPEN_HOUR, closeHour: CLOSE_HOUR, engineerCount: 0, calendar: [] });
  }
  ...
  const [{ data: windows }, { data: holidays }, { data: bookings }] = await Promise.all([
    admin.from("engineer_availability_windows").select("engineer_user_id, weekday, start_minute, end_minute").in("engineer_user_id", engineerIds),
    admin.from("engineer_holidays").select("engineer_user_id, holiday_date").in("engineer_user_id", engineerIds).gte("holiday_date", start.toISOString().slice(0, 10)).lt("holiday_date", end.toISOString().slice(0, 10)),
    admin.from("engineer_bookings").select("engineer_user_id, slot_start").in("engineer_user_id", engineerIds).eq("status", "booked").gte("slot_start", start.toISOString()).lt("slot_start", end.toISOString()),
  ]);

  type Win = { engineer_user_id: string; weekday: number; start_minute: number; end_minute: number };
  ...
  for (const h of (holidays ?? []) as { engineer_user_id: string; holiday_date: string }[]) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const wins = (winsByWeekday.get(weekday) ?? []).filter((w) => !holidaySet.has(`${w.engineer_user_id}|${dateStr}`));

    const coverageByHour = new Array(24).fill(0);
    for (let h = 0; h < 24; h++) {
      const hourStart = h * 60, hourEnd = h * 60 + 60;
      const covering = new Set<string>();
      for (const w of wins) {
        if (w.start_minute < hourEnd && w.end_minute > hourStart) covering.add(w.engineer_user_id);
      }
    ...
    for (let h = OPEN_HOUR; h < CLOSE_HOUR; h++) {
      if (coverageByHour[h] === 0) { if (gapStart === null) gapStart = h; }
      else if (gapStart !== null) { gaps.push({ startHour: gapStart, endHour: h }); gapStart = null; }
    }
    if (gapStart !== null) gaps.push({ startHour: gapStart, endHour: CLOSE_HOUR });

    calendar.push({ date: dateStr, weekday, coverageByHour, bookings: bookingsByDate.get(dateStr) ?? 0, gaps });
  }

  return NextResponse.json({ days, openHour: OPEN_HOUR, closeHour: CLOSE_HOUR, engineerCount: engineerIds.length, calendar });
}
