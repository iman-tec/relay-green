...
type PaymentIntent = { id: string; status: string; metadata: Record<string, string> };
...
export async function POST(request: Request) {
  ...
  const { quoteId, paymentIntentId } = (await request.json().catch(() => ({}))) as { quoteId?: string; paymentIntentId?: string };
  if (!quoteId || !paymentIntentId || !/^pi_/.test(paymentIntentId)) {
    return NextResponse.json({ error: "Missing quoteId or paymentIntentId." }, { status: 400 });
  }
  ...
  if (pi.status !== "succeeded") return NextResponse.json({ error: `Payment not completed (${pi.status}).` }, { status: 402 });
  if (pi.metadata?.relay_kind !== "contract" || pi.metadata?.relay_quote_id !== quoteId || pi.metadata?.relay_user_id !== user.id) {
    return NextResponse.json({ error: "Payment doesn't match this contract." }, { status: 400 });
  }

  const { error } = await admin
    .from("project_quote_requests")
    .update({ status: "committed", payment_intent_id: paymentIntentId, paid_at: new Date().toISOString(), committed_at: new Date().toISOString() })
    .eq("id", quoteId);
