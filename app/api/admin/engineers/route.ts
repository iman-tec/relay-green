...
export async function GET() {
  ...
  const { data: roleNames } = await admin
    .from("user_role_names")
    .select("user_id, role")
    .eq("role", ROLE.engineer);
  const engineerIds = [
    ...new Set((roleNames ?? []).map((r: { user_id: string }) => r.user_id)),
  ];
  ...
  const [
    ...
    admin
      ...
      .select(
        "user_id, display_alias, project_types, ai_tools, backend_stacks, frontend_stacks, experience_level, presence_state, is_available"
      )
  ...
  const nameById = new Map<
    string,
    { full_name: string | null; is_onboarded: boolean | null }
  >();
  ...
  for (const m of (podRows ?? []) as { user_id: string; pod_id: string }[])
    podByUser.set(m.user_id, podNameById.get(m.pod_id) ?? "—");

  const engineers = engineerIds.map((id) => {
    const ep = epById.get(id);
    const prof = nameById.get(id);
    const axesFilled = AXES.filter(
      (a) => Array.isArray(ep?.[a]) && (ep![a] as string[]).length > 0
    ).length;
    const hasLevel = !!ep?.experience_level;
    // 6-step intake ≈ 4 expertise axes + experience level + onboarded flag.
    const steps =
      axesFilled + (hasLevel ? 1 : 0) + (prof?.is_onboarded ? 1 : 0);
  ...
  const podList = ((pods ?? []) as { id: string; name: string }[]).map((p) => ({
    id: p.id,
    name: p.name,
  }));
