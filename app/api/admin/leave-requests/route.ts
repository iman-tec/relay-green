...
export async function GET() {
  ...
  const { data: reqs } = await admin
    .from("leave_requests")
    .select(
      "id, requester_user_id, requester_role, pod_id, start_date, end_date, total_days, reason, kind, created_at"
    )
    .eq("status", "pending")
  ...
  return NextResponse.json({
    requests: rows.map((r) => ({
      id: r.id,
      requester:
        nameById.get(r.requester_user_id) ??
        (r.requester_role === "supervisor" ? "Supervisor" : "Engineer"),
