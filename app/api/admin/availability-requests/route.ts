...
export async function GET() {
  ...
  const { data: reqs } = await admin
    ...
    .limit(100);
  const rows = (reqs ?? []) as { id: string; raised_by: string; engineer_user_id: string; pod_id: string | null; kind: string; detail: string | null; created_at: string }[];
  if (rows.length === 0) return NextResponse.json({ requests: [] });

  const userIds = [...new Set(rows.flatMap((r) => [r.raised_by, r.engineer_user_id]))];
