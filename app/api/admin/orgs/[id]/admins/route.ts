...
export async function POST(request: Request, { params }: RouteCtx) {
  ...
  if (!roleId) return NextResponse.json({ error: "enterprise_admin role not seeded" }, { status: 500 });
  ...
  if (body.promoteUserId) {
    ...
    const { data: prof } = await admin
      ...
      .maybeSingle();
    const p = prof as { id: string; organization_id: string | null; full_name: string | null } | null;
    if (!p || p.organization_id !== orgId) {
      return NextResponse.json({ error: "not_in_org" }, { status: 404 });
    }
    userId = p.id;
    displayName = p.full_name ?? "";
    await admin.from("profiles").update({ primary_role_id: roleId }).eq("id", userId);
    ...
    const { data: existingProfile } = await admin
      ...
      .eq("id", userId)
      .maybeSingle();
    const cp = existingProfile as { full_name: string | null; primary_role_id: string | null } | null;
