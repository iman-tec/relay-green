...
export async function GET(_request: Request, { params }: RouteCtx) {
  ...
  const { data: deptRow } = await admin
    .from("departments")
    .select(
      "id, enterprise_id, name, department_code, admin_user_id, allocated_minutes, used_minutes, remaining_minutes, status"
    )
    .eq("id", deptId)
    .maybeSingle();
  ...
  const { data: profRows, error: profErr } = await admin
    .from("profiles")
    .select(
      "id, full_name, primary_role_id, allocated_minutes, used_minutes, remaining_minutes, client_type, created_at"
    )
    .eq("department_id", deptId)
    .order("created_at", { ascending: false });
  if (profErr) {
    console.error(
      "[admin/orgs/departments/employees] profiles read failed:",
      profErr
    );
  ...
  const toRow = (p: Profile) => {
    ...
    return {
      ...
      email: a?.email ?? "",
      primaryRole: p.primary_role_id
        ? (roleNameById.get(p.primary_role_id) ?? null)
        : null,
  ...
  if (dept.admin_user_id && !adminProfile) {
    ...
    if (adminProfile && !authByUser.has(adminProfile.id)) {
      ...
      if (u?.user) {
        authByUser.set(adminProfile.id, {
          email: u.user.email ?? "",
          banned: Boolean(
            u.user.banned_until && new Date(u.user.banned_until) > new Date()
          ),
    ...
    if (
      adminProfile &&
      adminProfile.primary_role_id &&
      !roleNameById.has(adminProfile.primary_role_id)
    ) {
      const { data: rr } = await admin
        .from("roles")
        .select("id, name")
        .eq("id", adminProfile.primary_role_id)
        .maybeSingle();
...
export async function POST(request: Request, { params }: RouteCtx) {
  ...
  const { name, email, allocatedMinutes } = (await request
    .json()
    .catch(() => ({}))) as {
    name?: string;
    email?: string;
  ...
  const { data: deptRow } = await admin
    .from("departments")
    .select(
      "id, enterprise_id, name, department_code, status, remaining_minutes"
    )
    .eq("id", deptId)
    .maybeSingle();
  const dept = deptRow as {
  ...
  if (profileErr) {
    console.error(
      "[admin/orgs/departments/employees] profile upsert failed:",
      profileErr
    );
    return NextResponse.json(
      { error: `Profile link failed: ${profileErr.message}` },
      { status: 500 }
    );
  }
  ...
  if (
    !v ||
    v.department_id !== deptId ||
    v.organization_id !== orgId ||
    v.client_type !== "employee"
  ) {
  ...
  if (roleErr) {
    console.warn(
      "[admin/orgs/departments/employees] role upsert failed:",
      roleErr.message
    );
  }
  ...
  if (allocNum > 0) {
    ...
    if (tErr)
      console.warn(
        "[admin/orgs/departments/employees] initial transfer failed:",
        tErr.message
      );
  }
