/*
 * Admin-side department creation (mirrors /api/enterprise/departments POST
 * but gated on super_admin so the redesigned panel can manage departments
 * outside the org's own admin scope).
 *
 * POST /api/admin/orgs/:id/departments
 *   Body: { name, adminEmail?, adminDisplayName?, allocatedMinutes? }
 *   Creates the department. If admin fields are supplied, also invites a
 *   department_admin and transfers initial minutes (same logic as the
 *   enterprise-side endpoint). Otherwise creates an empty department that
 *   the org's enterprise_admin can populate later.
 */
...
export async function POST(request: Request, { params }: RouteCtx) {
  ...
  // GUARD: don't let a dept-admin email that belongs to another enterprise be
  // pulled in (it would move them out of their current org).
  {
  ...
  const orgRow = org as {
    id: string; name: string; enterprise_code: string; status: string; remaining_minutes: number;
  };
  ...
  if (allocNum > 0) {
    ...
    if (tErr) console.warn("[admin/orgs/departments] initial transfer failed:", tErr.message);
  }
