...
async function suiteInviteUrls() {
  ...
  const urlBlock = src.match(/SURFACE_URL\s*:\s*Record<LoginSurface,\s*string>\s*=\s*\{([\s\S]*?)\}\s*as const/);
  if (!urlBlock) {
    throw new Error("Could not locate SURFACE_URL definition in lib/relay/loginSurface.ts");
  }
  const urls = Object.fromEntries(
    [...urlBlock[1].matchAll(/(\w+)\s*:\s*"([^"]+)"/g)].map(([_, k, v]) => [k, v]),
  );
  ...
  const roles = ["client", "reseller", "enterprise_admin", "department_admin", "super_admin", "supervisor", "engineer", null];
  ...
  const { data: row, error: insErr } = await admin
    .from("invites")
    .insert({
      email, name: null, role: "client",
      scope_type: scopeType, scope_id: scopeId,
      department_id: dept?.id ?? null,
      code, invited_by: "00000000-0000-0000-0000-000000000000",
    })
    .select("id, role, code, email")
    .maybeSingle();
  if (insErr || !row) {
    console.log(`  (skipped real-row check — insert error: ${insErr?.message ?? "unknown"})`);
  } else {
    const brand = process.env.NEXT_PUBLIC_BRAND_DOMAIN || "relay.green";
    ...
    check(`DB-row role=${row.role} → built link is "${expected}?invite=…"`,
      expectedLink.includes(`${expected}?invite=`), expectedLink);
    // Cleanup
