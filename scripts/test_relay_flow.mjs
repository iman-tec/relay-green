...
const ENV_PATH =
  "/home/devsoni/Pictures/relay/relay.green.claude/.claude/worktrees/clever-colden-d6a715/relay-green/.env.local";
...
log(
  `  ${PASS} session created: ${session.id.slice(0, 8)}…  status=${session.status}`
);
...
// Verify idempotency
const { data: again } = await customerClient.rpc(
  "get_or_create_active_customer_session"
);
...
// Should rate-limit
const { error: rlErr } = await customerClient.rpc("recall_engineer", {
...
const { data: afterRecalls } = await customerClient
  .from("guest_calls")
  .select("recall_count, urgency")
  .eq("id", session.id)
  .single();
if (afterRecalls.recall_count !== 3)
  fatal("expected recall_count=3", afterRecalls.recall_count);
if (afterRecalls.urgency !== "urgent")
  fatal(`expected urgency=urgent (≥3 recalls), got=${afterRecalls.urgency}`);
log(
  `  ${PASS} urgency escalated to '${afterRecalls.urgency}' (recall_count=${afterRecalls.recall_count})`
);
...
const { data: claimed, error: cErr } = await engineerClient.rpc(
  "claim_session",
  {
    _session_id: session.id,
  }
...
if (claimedSession.status !== "assigned")
  fatal("expected assigned", claimedSession.status);
...
await customerClient.rpc("mark_joined", {
  _session_id: session.id,
  _role: "customer",
});
const { data: joining } = await customerClient
  .from("guest_calls")
  .select("status, customer_joined_at")
  .eq("id", session.id)
  .single();
...
await engineerClient.rpc("mark_joined", {
  _session_id: session.id,
  _role: "engineer",
});
const { data: live } = await customerClient
  .from("guest_calls")
  .select("status, joined_at, engineer_joined_at")
  .eq("id", session.id)
  .single();
if (live.status !== "live") fatal("expected live", live.status);
if (!live.joined_at) fatal("joined_at not set");
log(
  `  ${PASS} engineer joined → status=${live.status}, joined_at=${live.joined_at}`
);
...
const { data: ent } = await customerClient
  ...
  .select("*")
  .eq("customer_user_id", cu.user.id)
  .single();
...
log(
  `  ${PASS} ${guestMsgs.length} guest, ${engMsgs.length} engineer, ${sysMsgs.length} system messages`
);
...
if (endedSession.status !== "ended")
  fatal("expected ended", endedSession.status);
if (endedSession.ended_reason !== "test_complete") fatal("ended_reason wrong");
log(
  `  ${PASS} end → status=${endedSession.status}, duration=${Number(endedSession.duration_minutes).toFixed(2)}min`
);

// Idempotent
const { data: ended2 } = await engineerClient.rpc("end_session", {
  _session_id: session.id,
});
...
audit.forEach((a) =>
  log(
    `    · ${a.action}${a.from_state ? `  ${a.from_state} → ${a.to_state}` : ""}`
  )
);
