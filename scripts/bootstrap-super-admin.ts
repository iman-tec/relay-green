...
async function bootstrapOne(
  ...
  if (profileErr) {
    throw new Error(
      `Upserting profile failed for ${entry.email}: ${profileErr.message}`
    );
  }
  ...
  const { error: rpcErr } = await admin.rpc("bootstrap_super_admin", {
    _user_id: userId,
  });
  if (rpcErr) {
    if (rpcErr.message.includes("SUPER_ADMIN_ALREADY_EXISTS")) {
      ...
      if (insertErr) {
        throw new Error(
          `Granting super_admin failed for ${entry.email}: ${insertErr.message}`
        );
      }
    } else {
      throw new Error(
...
async function main() {
  ...
  console.log("");
  console.log(
    `✓ Bootstrapped ${SUPER_ADMINS.length} Super Admin${SUPER_ADMINS.length === 1 ? "" : "s"}.`
  );
  console.log(
    `  Sign in at /staff with any of the listed emails → enter the OTP from your inbox.`
  );
}
