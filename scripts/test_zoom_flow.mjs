...
if (!eng) { console.error(`${FAIL} engineer not found — run scripts/reset.mjs first`); process.exit(1); }
await admin.auth.admin.updateUserById(eng.id, { password: PASSWORD });
const engClient = createClient(URL, ANON);
const { data: sign, error: sErr } = await engClient.auth.signInWithPassword({ email: ENG_EMAIL, password: PASSWORD });
if (sErr) { console.error(`${FAIL} sign-in failed`, sErr); process.exit(1); }
console.log(`  ${PASS} engineer signed in`);

// ── Step 2: Make a fake customer + session ─────────────────────────────
console.log(`\n${STEP} Creating a fake queued session`);
const { data: sess } = await admin.from("guest_calls").insert({
...
const { data: claim, error: cErr } = await engClient.rpc("claim_session", { _session_id: sess.id });
if (cErr) { console.error(`${FAIL} claim failed`, cErr); process.exit(1); }
const claimed = Array.isArray(claim) ? claim[0] : claim;
console.log(`  ${PASS} status=${claimed.status}, claimed_by=${claimed.claimed_by.slice(0, 8)}…`);

// ── Step 4: Mint Zoom meeting via edge fn ──────────────────────────────
console.log(`\n${STEP} Calling mint-zoom-for-session edge fn`);
const t0 = Date.now();
const { data: mint, error: mErr } = await engClient.functions.invoke("mint-zoom-for-session", {
...
const { data: row } = await admin.from("guest_calls").select("zoom_meeting_id, zoom_join_url, zoom_start_url, status").eq("id", sess.id).single();
if (!row?.zoom_meeting_id) { console.error(`${FAIL} row missing zoom_meeting_id`); process.exit(1); }
...
const t1 = Date.now();
const { data: sig1, error: sig1Err } = await engClient.functions.invoke("zoom-sdk-signature", {
...
console.log(`     signature: ${sig1.signature?.slice(0, 32)}…  (length=${sig1.signature?.length})`);
console.log(`     password:  ${sig1.password ? "(present)" : "(empty)"}`);
console.log(`     zak:       ${sig1.zak ? "(present — required for host)" : "(empty)"}`);
if (!sig1.zak) {
  console.log(`     ${FAIL} zak missing — host role won't start the meeting`);
  console.log(`        Verify ZOOM_SDK_KEY/ZOOM_SDK_SECRET match a Zoom Meeting SDK app`);
  console.log(`        AND that the Server-to-Server OAuth user has start-meeting scope`);
}

// ── Step 7: Test zoom-sdk-signature (role=0 attendee) ─────────────────
console.log(`\n${STEP} Calling zoom-sdk-signature (role=0 attendee)`);
const { data: sig0, error: sig0Err } = await engClient.functions.invoke("zoom-sdk-signature", {
