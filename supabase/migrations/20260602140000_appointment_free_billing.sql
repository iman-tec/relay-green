-- ============================================================================
-- Appointment calls are FREE
-- ============================================================================
-- A call started from a scheduled appointment (guest_calls.is_appointment) must
-- never bill the customer: no paid-minute deduction, no free-quota consumption,
-- no employee-pool draw, and no paywall (it never flips to expired_free).
--
-- This replaces end_session() to short-circuit ALL billing when is_appointment,
-- and expire_to_free() to no-op for appointments. Both are gated strictly on
-- is_appointment, so the normal engineer↔customer billing flow is unchanged.
--
-- Replaces two function bodies only. Nothing dropped.
-- ============================================================================
...
BEGIN
  ...
  -- Paid minutes for THIS session. Appointments are FREE → always 0.
  IF _s.is_appointment OR NOT _was_assigned THEN
    _paid_min := 0;
  ...
  PERFORM _log_session_event(_session_id, 'session.ended',
    ...
    jsonb_build_object(
      ...
      'paid_min',     _paid_min,
      'billing_path', CASE WHEN _s.is_appointment THEN 'appointment_free'
                           WHEN _is_employee THEN 'employee' ELSE 'legacy' END,
      'anchor',       'assigned_at'
    )
  );

  -- Appointments bill nothing: skip employee-pool draw, free-quota consumption,
  -- and wallet deduction entirely.
  IF NOT _s.is_appointment THEN
...
-- ── expire_to_free: never expire an appointment (no paywall) ────────────────
...
BEGIN
  ...
  -- Appointments are free → never hit the paywall.
  IF _s.is_appointment THEN
    RETURN _s;
  END IF;
