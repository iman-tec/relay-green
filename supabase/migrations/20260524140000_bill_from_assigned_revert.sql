-- ============================================================================
-- end_session: revert billing anchor to assigned_at (session/chat start)
-- ============================================================================
-- 20260524130000 moved the billing anchor to engineer_joined_at (Zoom join).
-- That was the wrong reading of the requirement: per product feedback the
-- session clock must start when the session/chat starts — i.e. when the
-- engineer ACCEPTS/connects (assigned_at) — and must NOT depend on the Zoom
-- call. The Zoom-anchored version made starting a Zoom call mid-session reset
-- the timer to 0 and refund consumed minutes, which is wrong.
--
-- This restores the assigned_at anchor (the 20260521200000 behaviour).
-- assigned_at is set the moment the engineer accepts (accept_match) — for
-- admin assignment that now happens via a directed ring + accept too
-- (20260524150000), so assigned_at always marks the engineer actually joining
-- the session, never the bare admin action.
-- ============================================================================
...
BEGIN
  ...
  -- "Session started" = engineer accepted/connected (assigned_at). Anchors
  -- duration, free quota consumption, and paid billing. Sessions that never
  -- reached this state (queue-only) bill nothing and don't burn the free quota.
  ...
  ELSIF _free_was_consumed_before THEN
    _paid_min := GREATEST(0::numeric, EXTRACT(EPOCH FROM (now() - _s.assigned_at)) / 60.0);
