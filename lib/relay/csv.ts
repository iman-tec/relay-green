 ...
 */
const COMPANY_REQUIRED_COLS = ["email", "name", "company", "discount", "months"] as const;
...
export function parseCompaniesCsvStrict(text: string): CompaniesParseResult {
  ...
  const normalized = headerCells.map((h) => (h === "company_name" ? "company" : h));
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (rowProblems.length > 0) {
      result.rowErrors.push(`Row ${rowNum}: missing ${rowProblems.join(", ")}.`);
      continue;
    }
    ...
    if (discRaw === "") {
      ...
      if (!Number.isFinite(parsed) || parsed < 0 || parsed > 100) {
        result.rowErrors.push(`Row ${rowNum}: discount must be a number between 0 and 100 (got "${discRaw}").`);
        continue;
      }
      discountPct = parsed;
    }
    ...
    if (!Number.isFinite(discountMonths) || discountMonths < 1 || discountMonths > 36) {
      result.rowErrors.push(`Row ${rowNum}: months must be a whole number between 1 and 36 (got "${monRaw}").`);
      continue;
    }
