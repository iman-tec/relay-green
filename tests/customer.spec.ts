...
test.describe("customer /room", () => {
  test("creates a queued session on first land", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: /Connecting you with the best engineer/i,
      })
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Call for engineer/i })
    ).toBeVisible();
  ...
  test("recall button increments count and escalates to urgent at >= 3", async ({
    page,
  }) => {
    const user = await createTestUser("customer");
    ...
    await expect(page.getByText(/Recalled 1 time/i)).toBeVisible({
      timeout: 10_000,
    });
    ...
    await expect(page.getByText(/Recalled 2 times/i)).toBeVisible({
      timeout: 10_000,
    });

    await bypassRateLimit(session!.id);
    await page.getByRole("button", { name: /Recall engineer/i }).click();
    await expect(page.getByText(/Recalled 3 times/i)).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("rate-limited recall shows cooldown message", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Wait \d+s before recalling/ })
  ...
  test("when engineer claims, customer sees 'Engineer is ready' modal", async ({
    page,
  }) => {
    const user = await createTestUser("customer");
    await deleteUserSessions(user.id);
