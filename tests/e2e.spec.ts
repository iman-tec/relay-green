...
test("E2E lifecycle: customer queues → engineer claims → both join → LIVE → end → post-call", async () => {
  ...
  try {
    ...
    await expect(
      engPage.getByText(cust.email.split("@")[0]).first()
    ...
    try {
      await expect(
        custPage.getByRole("heading", { name: /is ready/i })
    ...
    await expect(engPage.getByText(/Live/).first()).toBeVisible({
      timeout: 10_000,
    });
    ...
    await expect(engPage.getByText(/Chat history/i)).toBeVisible({
      timeout: 15_000,
    });
    await expect(engPage.getByText(/AI summary/i)).toBeVisible();

    // Customer sees post-call split too
    await expect(custPage.getByText(/Chat history/i)).toBeVisible({
      timeout: 15_000,
    });
