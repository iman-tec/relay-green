...
test.describe("public pages render", () => {
  test("landing page shows brand + Try Relay CTA", async ({ page }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: /Build with AI/i })
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Try Relay/i }).first()
    ).toBeVisible();
  ...
  test("/login renders OTP form", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Already have a code/i })
    ).toBeVisible();
  });
  ...
  test("Try Relay button navigates to /login", async ({ page }) => {
    await page.goto("/");
    await page
      .getByRole("button", { name: /Try Relay/i })
      .first()
      .click();
