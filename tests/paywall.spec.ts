...
async function creditPaidMinutes(userId: string, minutes: number) {
  ...
  await admin
    .from("credit_wallets")
    .upsert({ user_id: userId, balance: minutes });
}
...
test.describe("paywall + entitlement lifecycle", () => {
  ...
  test("fresh customer sees 10 min free available and no past sessions", async ({
    page,
  }) => {
    ...
    await expect(page.getByText(/10 min free available/i)).toBeVisible({
      timeout: 10_000,
    });
    // Sidebar has no past sessions
    await expect(
      page.getByText(/Your past sessions will appear here/i)
    ).toBeVisible();
    // Paywall is NOT open
  ...
  test("ended session appears in sidebar even after only seconds", async ({
    page,
  }) => {
    const user = await createTestUser("customer");
    await deleteUserSessions(user.id);
    ...
    await expect(
      page.getByRole("heading", { name: /Connecting/i })
    ...
    await page.reload();
    await expect(page.getByText(/Test ended session/i)).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("exhausted customer lands and paywall auto-opens (no NO_ENTITLEMENT toast)", async ({
    page,
  }) => {
    const user = await createTestUser("customer");
    await deleteUserSessions(user.id);
    ...
    await expect(page.getByText(/Three phases\. One team\./i)).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("closing paywall and clicking + New session re-opens it", async ({
    page,
  }) => {
    const user = await createTestUser("customer");
    await deleteUserSessions(user.id);
  ...
  test("profile chip click opens paywall when exhausted", async ({ page }) => {
    ...
    const profileBtn = page
      .locator("button", { hasText: user.email.split("@")[0] })
      .last();
  ...
  test("Stripe success URL shows payment-received toast and cleans URL", async ({
    page,
  }) => {
    const user = await createTestUser("customer");
    await deleteUserSessions(user.id);
    ...
    await admin
      .from("credit_wallets")
      .upsert({ user_id: user.id, balance: 60 });
    await authPage(page, user);
    ...
    await expect
      ...
      .not.toContain("relay_paid");
    // Profile chip shows paid balance
    await expect(page.getByText(/60 min paid/i)).toBeVisible({
      timeout: 8_000,
    });
  });

  // Paid customer with credit can start a new session (no paywall blocking).
  test("paid customer can start new session without paywall", async ({
    page,
  }) => {
    const user = await createTestUser("customer");
    await deleteUserSessions(user.id);
    ...
    await expect(page.getByText(/120 min paid/i)).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("ended session under 30s does NOT consume free quota", async () => {
    ...
    const sb = createClient(SUPABASE_URL, ANON_KEY);
    await sb.auth.setSession({
      access_token: eng.accessToken,
      refresh_token: eng.refreshToken,
    });
    const { error: endErr } = await sb.rpc("end_session", {
      _session_id: sessionId,
      _reason: "customer_ended",
    });
  ...
  test("ended session ≥ 30s DOES consume free quota", async () => {
    ...
    const { data: ent } = await admin
      ...
      .maybeSingle();
    const e = ent as {
      free_session_consumed_at: string | null;
      free_session_id: string | null;
    } | null;
