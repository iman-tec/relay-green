...
import {
  createTestUser, authPage, cleanupTestUsers, admin,
  deleteUserSessions, getActiveSession,
...
async function dbStatus(sessionId: string): Promise<string> {
  const { data } = await admin.from("guest_calls").select("status").eq("id", sessionId).single();
...
test.describe("engineer attends queued sessions (full UI flow)", () => {
  ...
  test("engineer claims via /inbox → both join → live → engineer ends", async () => {
    ...
    try {
      ...
      await expect.poll(async () => {
        const { data } = await admin.from("guest_calls")
          .select("status, claimed_by").eq("id", session!.id).single();
        const row = data as { status: string; claimed_by: string | null } | null;
        return `${row?.status}|${row?.claimed_by}`;
      }, { timeout: 10_000 }).toBe(`assigned|${eng.id}`);
      ...
      try {
        await expect(custPage.getByRole("heading", { name: /is calling/i }))
      ...
      await expect.poll(() => dbStatus(session!.id), { timeout: 15_000 }).toBe("live");

      // ── Step 8: engineer clicks End session in the header pill ──────────
      await engPage.getByRole("button", { name: /^End session$/i }).first().click();
      // Confirmation modal — click the modal's End session button
      await engPage.getByRole("button", { name: /^End session$/i }).last().click();

      // ── Step 9: DB confirms ended ───────────────────────────────────────
      await expect.poll(() => dbStatus(session!.id), { timeout: 15_000 }).toBe("ended");
    } finally {
      await custCtx.close();
      await engCtx.close();
      await browser.close();
  ...
  test("/inbox Take next waiting call claims the queued session", async ({ page }) => {
    const cust = await createTestUser("customer");
    const eng  = await createTestUser("engineer");
    ...
    try {
      ...
      await expect(page.getByText(/^Incoming request$/i)).toBeVisible({ timeout: 15_000 });
      await page.getByRole("button", { name: /Dismiss/i }).click();
      // Wait for the queue to show this customer's row before clicking Take next.
      await expect(page.getByText(cust.email.split("@")[0]).first()).toBeVisible({ timeout: 15_000 });
      await page.getByRole("button", { name: /Take next waiting call/i }).click();
      await expect(page).toHaveURL(/\/staff\/session\/[0-9a-f-]+/, { timeout: 10_000 });

      await expect.poll(async () => {
        const { data } = await admin.from("guest_calls").select("claimed_by").eq("id", session!.id).single();
        return (data as { claimed_by: string | null } | null)?.claimed_by;
      }, { timeout: 10_000 }).toBe(eng.id);
    } finally {
      await custCtx.close();
      await browser.close();
    }
  ...
  test("/inbox Take next race: both engineers claim, no one gets stranded", async () => {
    ...
    const [{ data: s1 }, { data: s2 }] = await Promise.all([
      admin.from("guest_calls").insert({
        guest_name: "Race-1", guest_email: cust1.email, status: "queued",
        customer_user_id: cust1.id, free_minutes: 10,
      }).select().single(),
      admin.from("guest_calls").insert({
        guest_name: "Race-2", guest_email: cust2.email, status: "queued",
        customer_user_id: cust2.id, free_minutes: 10,
      }).select().single(),
    ]);
    ...
    try {
      ...
      await expect(eng1Page.getByText(/relay-pw-customer-/i).first()).toBeVisible({ timeout: 15_000 });
      await expect(eng2Page.getByText(/relay-pw-customer-/i).first()).toBeVisible({ timeout: 15_000 });

      // Both click "Take next" at the same time
      await Promise.all([
        eng1Page.getByRole("button", { name: /Take next waiting call/i }).click(),
        eng2Page.getByRole("button", { name: /Take next waiting call/i }).click(),
      ]);

      // Each ends up on a different session URL (no one stranded)
      await expect(eng1Page).toHaveURL(/\/staff\/session\/[0-9a-f-]+/, { timeout: 15_000 });
      await expect(eng2Page).toHaveURL(/\/staff\/session\/[0-9a-f-]+/, { timeout: 15_000 });

      // DB: both sessions claimed, by different engineers
      const { data: rows } = await admin.from("guest_calls")
        .select("id, claimed_by, status")
        .in("id", [s1Id, s2Id]);
      const list = (rows ?? []) as Array<{ id: string; claimed_by: string | null; status: string }>;
      expect(list.length).toBe(2);
      const claimers = list.map((r) => r.claimed_by).filter((x): x is string => !!x).sort();
  ...
  test("concurrent claim: only one of two engineers wins", async () => {
    ...
    const { data: insertData, error: insertErr } = await admin.from("guest_calls").insert({
    ...
    try {
      ...
      const [r1, r2] = await Promise.all([
        eng1Page.request.post(`/api/test/echo`, { failOnStatusCode: false }).then(() => null).catch(() => null),
        eng2Page.request.post(`/api/test/echo`, { failOnStatusCode: false }).then(() => null).catch(() => null),
      ]);
      void r1; void r2;
      ...
      await sb1.auth.setSession({ access_token: eng1.accessToken, refresh_token: eng1.refreshToken });
      await sb2.auth.setSession({ access_token: eng2.accessToken, refresh_token: eng2.refreshToken });
      ...
      const { data: final } = await admin.from("guest_calls").select("claimed_by, status").eq("id", sessionId).single();
